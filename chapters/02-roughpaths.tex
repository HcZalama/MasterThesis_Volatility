...
What follows is not a survey of an unrelated theory placed alongside Part \ref{part:part1}. Three of
the central results of Chapter \ref{ch:foundations} --- Itô's formula, the Itô--Stratonovich
correction, and the Wong--Zakai approximation theorem --- will be \emph{re-derived} here on
...
deterministically, and with a continuity statement attached --- and that the second-order term which
Chapter \ref{ch:foundations} presented as an irreducible feature of stochastic calculus turns out to
be a consequence of a choice. The coda closing the chapter collects the three and draws the moral.
...
\eqref{eq:controlleddef}. \hfill $\blacksquare$
\end{theorem}
...
\begin{itemize}
    ...
    \end{equation}
    which is the case $\theta(t,x) = F(x)$, $X = W$ of Theorem \ref{thm:itomulti} of Chapter
    \ref{ch:foundations}.
\end{itemize}
\end{corollary}
...
Taken together these say something the classical theory cannot. In Chapter \ref{ch:foundations} the
second-order term of Itô's rule, the $\tfrac12\int F'$ of the Itô--Stratonovich dictionary, and the
failure of smooth approximation to converge to the Itô solution appear as three separate facts,
