...
Brownian motion has finite $p$-variation exactly for $p>2$; Young's integral requires
...
in place of a pathwise one, and a definition valid only up to null sets. This allowed us to construct solutions, but we paid a double cost.
...
would naïvely choose. A model whose output is a discontinuous
function of its input is a model one cannot approximate, cannot discretise with confidence, and
cannot calibrate stably. Rough path theory, introduced by Lyons \cite{lyons1998}, resolves both difficulties simultaneously,
...
independence of those increments. Relaxing exactly that requirement yields
...
\ref{thm:kolmogorovcontinuity}) supplies a continuous modification.
...
d) Finiteness for $p>1/\Hurst$ follows from c); the divergence for $p\leq 1/\Hurst$ requires the
same competition between $p$-variation and modulus of continuity that produced Proposition
\ref{prop:bmunboundedvariation}. See \cite[Ch.\ 15]{frizvictoir2010} for the details, which we do
not repeat.
...
direction. Only the balance point $\Hurst = \tfrac12$ escapes. The consequence is severe: for
$\Hurst \neq \tfrac12$ none of the semimartingale apparatus of Chapter \ref{ch:foundations} is
available --- no Itô integral against $\BH$, no Itô formula, no martingale representation. Other
integration theories for $\BH$ do exist, but they are not the theory built in Chapter
\ref{ch:foundations}.

\subsection{Volatility is Rough}\label{subsec:roughvol}

Let $\sigma_t$ denote realised volatility, estimated from high-frequency returns over a grid of
...
indices, $m(q,\Delta) \propto \Delta^{\zeta_q}$ with
...
persistent, it is \emph{rough}. The resulting \textbf{rough fractional stochastic volatility}
(RFSV) model replaces the Heston dynamics of Subsection \ref{subsec:stochvol} by
...
A model of the form \eqref{eq:rfsv} generates an at-the-money skew whose term structure behaves like
...
\end{equation}
a power-law explosion at short maturities, where a continuous semimartingale volatility factor gives
a skew with a finite limit. This is the one place where $\Hurst$ becomes visible in option prices
rather than in realised volatility, and it is what \S\ref{subsec:rhidentification} sets out to
measure.
...
\noindent\textbf{Young integration is out of reach.} Recall
...
\noindent\textbf{The obstruction is not a matter of technique.} One might still hope that the
...
data, postulate it where it cannot be constructed, and topologise the enlarged object. 
...
The iterated integrals we must carry are indexed by pairs of coordinates, so the algebraic home
...
\cite[Ch.\ 3]{geng2021} and \cite[Ch.\ 7]{frizvictoir2010}.
...
\end{equation}
all terms of higher order being discarded. We write
...
$N$, and the full tensor algebra is $T((V)) := \prod_{n \geq 0} V^{\otimes n}$. Only $\Tens{2}$ is used until
...
the whole chapter takes place. The motivation is the following computation, which the reader should regard as the definition of
...
\end{equation}
both integrals being ordinary Riemann--Stieltjes integrals, so
...
\]
and integrating from $s$ to $t$, where the left-hand side vanishes at $r=s$, yields the claim. \qedfilledtext

\medskip

So for a smooth path the symmetric part of the iterated integral carries no information beyond the
increment itself: only the \emph{antisymmetric} part, the \textbf{Lévy area}
...
\medskip
At the first level, 
$X_{s,t} = X_{s,u} + X_{u,t}$ is just the additivity of increments. At the second level it is a genuine
...
telescope, so $\lev$ is not the increment of any $V\otimes V$-valued path. The first level, by contrast, may be
...
Definition \ref{def:pvariation}, in which Proposition \ref{prop:fbmproperties} d) is also stated, so that a reader arriving here expects it. The two
...
$\rpdist(\rpX,\mathbf{0}) \neq \vertiii{\rpX}_\alpha$. The metric is
...
This is the single most important structural fact in the subject: \textbf{a rough path is not a path.} The second level is genuine additional data, undetermined by
...
proceeds. Different choices are different rough paths, and they yield different integrals and different solutions to differential
equations. 
...
\end{equation}
where $\mathrm{Id} = \sum_i e_i \otimes e_i$. Then, for $\alpha \in (\tfrac13,\tfrac12)$, both
...
notation this reads precisely  $2\,\Sym\levW^{\mathrm{It\hat o}}_{s,t} = W_{s,t}\otimes W_{s,t} - (t-s)\mathrm{Id}$,
so \eqref{eq:weaklygeometric} fails for the Itô lift by $-\tfrac12(t-s)\mathrm{Id}$, and
...
\end{equation}
Unconditionally, $A_{0,t}$ is distributed as $t\,C$ with $C$ having characteristic
function $1/\cosh(u/2)$, so that $\Var[A_{0,t}] = t^{2}/4$. Note that the law is \emph{not} Gaussian, which is the first sign that the
...
The Itô--Stratonovich correction of Chapter
...
Theorem \ref{thm:fbmlift} covers $\Hurst>\tfrac13$; a third iterated integral extends the range to
$\Hurst>\tfrac14$ and no further, the piecewise-linear lifts failing to converge at any number of
levels below that. The exponent $\Hurst\approx0.1$ of Subsection \ref{subsec:roughvol} therefore
lies outside the canonical construction --- not because no lift exists, since Lyons--Victoir
\cite{lyonsvictoir2007} supply one for every $\alpha$-Hölder path, but because none of them is the
limit of the discretisations one would compute with. Chapter \ref{ch:applications} is built so as
not to cross that boundary: rough Heston is reached through the affine route of Theorem
\ref{thm:roughhestoncf}, and the signature of Section \ref{sec:signature} is evaluated on sampled,
piecewise-linear data, which lifts canonically whatever the regularity of the process it samples.
\end{remark}
...
Every construction of an integral in this dissertation has had
the same shape --- propose a Riemann sum, show it converges. The following lemma, due in this form
to Gubinelli, extracts the mechanism.
...
genuinely additive one. The strictness of $\beta>1$ is what the counting argument above consumes: at $\beta=1$ the factor $n^{1-\beta}$ is $1$ and the argument collapses.
...
We cannot integrate an arbitrary path against
...
that appear in differential equations.
\end{example}

\medskip

The naïve Riemann sum $\sum Y_u X_{u,v}$ does not
...
\textbf{\textit{Proof:}} An application of Lemma \ref{lem:sewing} to
$\Xi_{s,t} := Y_sX_{s,t} + \Gub_s\lev_{s,t}$; the argument is given in Appendix
\ref{app:proof:gubinelli}. \qedfilledtext
...
Stratonovich integral of Definition \ref{def:stratonovich} respectively. \qedopentext
\end{proposition}
...
\begin{corollary}[Itô's formula]\label{cor:itoformula}
...
\begin{itemize}
    ...
    \ref{ch:foundations}. \qedfilledtext
\end{itemize}
\end{corollary}
...
different foundations}. What is bought in exchange is that \eqref{eq:itofromrough} now holds pathwise,
...
In Chapter \ref{ch:foundations}, the second-order term of Itô's formula arose from
...
trajectory has no such term at all. The correction was therefore not forced by Brownian motion
alone: the non-vanishing quadratic variation is what makes the two lifts differ at all --- for a
driver of bounded variation they coincide --- but which of them is taken is
a choice, made implicitly in Subsection \ref{subsec:itointegral} when the integrand
...
Let $\rpX \in \RP{\alpha}$, $\xi \in W$ and $f : W \to \Lloc{V,W}$. A pair
...
Without it uniqueness fails outright. Consider $X\in\Holder{1}$
...
\textbf{\textit{Proof:}} (Sketch; the argument is given in full in Appendix
\ref{app:proof:rdeexistence}.) One defines the map
...
This is precisely the Picard iteration on which Theorem \ref{thm:sdeexistence} rests in Chapter
\ref{ch:foundations}, and the analogy is exact --- but the argument is now entirely deterministic, and
takes place in the space of controlled paths rather than in $L^2(\Om)$. \qedfilledtext
...
We directly state one of the key results of rough path theory.
...
\textbf{\textit{Proof:}} The argument is given in Appendix \ref{app:proof:ult}; it combines the
stability of rough integration (Theorem \ref{thm:integralstability}) with the stability of
composition, applied to the fixed point characterisation \eqref{eq:picardmap}. \qedfilledtext
...
driving signal is \emph{enhanced}, the solution map becomes not merely continuous but locally Lipschitz, and entirely
...
\ref{ex:brownianroughpath}.

Three consequences deserve mention. The first is the following theorem:

\begin{corollary}[Wong--Zakai]\label{cor:wongzakai}
Let $W$ be a Brownian motion, let $W^{(n)}$ be its piecewise-linear interpolation over a partition of mesh
tending to zero, and let $Y^{(n)}$ solve the ordinary differential equation
$\dd Y^{(n)} = f(Y^{(n)})\,\dd W^{(n)}$. Then $Y^{(n)}$ converges in probability, in
$\alpha$-Hölder norm --- and almost surely along a nested sequence of partitions, the dyadic one for
instance --- to the solution of the \emph{Stratonovich} equation
$\dd Y = f(Y)\circ \dd W$.
\end{corollary}
...
geometric, hence so is the limit by Proposition \ref{prop:geometricinclusion}; that the limit is
$\mathbf{W}^{\mathrm{Strat}}$ specifically, and not some other geometric lift, is part of the cited
result. Theorem
...
Second, Theorem \ref{thm:ult} puts numerical analysis on a pathwise footing. The Milstein scheme
\eqref{eq:davie} converges because the map is continuous, and the rate is controlled by
\eqref{eq:ult}; within Itô theory the corresponding estimates are made in $L^2(\Om)$, and afresh
for each scheme.
...
covariance, suitable Markov processes such as uniformly elliptic symmetric diffusions, and the
piecewise-linear paths formed from observed data.
...
Three properties make the signature useful:
...
factorially, so a truncation at a modest $N$ retains almost all
...
polynomial in the entries of $\Sig(\rpX)$ is again a linear functional of $\Sig(\rpX)$: the linear
span of the coordinates is already an algebra. That much is pure algebra and uses nothing beyond
\eqref{eq:shuffle}. The statement it is wanted for --- that \emph{any} continuous functional of the
path is a limit of linear functionals of its signature --- needs one further ingredient, namely that
the signature separates the paths it is asked to tell apart. That is Theorem
\ref{thm:signatureuniqueness} below, and we return to the approximation statement once it and the
device it depends on are available.
...
Back in Chapter \ref{ch:foundations}, discussing the log-normal
...
moment with the log-normal: knowing every moment of an object is strictly weaker than knowing the
object. Theorem \ref{thm:signatureuniqueness} is that statement for \textit{paths}. The iterated
...
path is traversed carries meaning.

The approximation statement promised after Proposition \ref{prop:shuffle} can now be made. On a
compact family of time-augmented paths the coordinates of the signature separate points, by Theorem
\ref{thm:signatureuniqueness} together with the time augmentation just described; their span
contains the constants and, by the shuffle relation \eqref{eq:shuffle}, is an algebra. The
Stone--Weierstrass theorem therefore makes every continuous functional of the path a uniform limit
of \emph{linear} functionals of its signature. A nonlinear problem on path space becomes a linear
problem on signature space, and this is the theoretical basis for the use of signatures as features
in statistical learning; see \cite{chevyrevkormilitzin2016} and \cite{levinlyonsni2013}. 
...
plays in Appendix \ref{app:heyde}.  At the level of individual objects the correspondence is between the power $X^k$ of a random
...
real number determine it trivially. 
...
transform; Proposition \ref{prop:factorialdecay} bounds the tail discarded by truncation, which is
cheap once $N$ exceeds the length of the path, and is set against a count of words growing like
$d^{N}$; Proposition
...
% And, as noted in Remark \ref{rem:H01outside}, none of the three requires a
...
\section{Rough Models}\label{sec:roughvolmodels}
...
these requirements is the rough counterpart of the Heston model of Subsection
\ref{subsec:stochvol}, whose tractability rested on its affine structure.
...
Feynman--Kac representation. The second is a survival, and it is what makes the model usable. The \emph{affine} structure is untouched by
...
Let $\Fwd$ denote the forward price. Under \eqref{eq:roughheston} the
characteristic function of the centred log-forward, stated at $t=0$, is
\begin{equation}\label{eq:roughhestoncf}
\charf(u,T) \;=\; \Ep{\QQ}\Bigl[ \exp\Bigl(iu \log\tfrac{\Fwd_T}{\Fwd_0}\Bigr) \Bigr]
\;=\; \exp\Bigl( \kappa\theta \int_0^{T}\psi(u,s)\,\dd s \;+\; v_0\, I^{1-\alpha_\Hurst}\psi(u,T) \Bigr),
\end{equation}
...
The statement is made at $t=0$ deliberately. By the loss recorded above, the time-$t$ conditional
law depends on the whole forward variance curve $s\mapsto\Ep{\QQ}[v_s\mid\Ft]$ and not on $v_t$
alone, so \eqref{eq:roughhestoncf} does not survive being conditioned on $\Ft$ with $v_t$ in place
of $v_0$.
...
\eqref{eq:hestonriccati} \emph{term for term}: the single
difference is that the ordinary derivative $\partial_\tau$ has been replaced by a fractional
derivative of order $\alpha_\Hurst = \Hurst+\tfrac12$, and at $\Hurst = \tfrac12$ the two coincide. We shortly introduce one of the many notions of fractional derivative. 
...
\begin{definition}[Riemann--Liouville fractional operators]\label{def:fracderiv}
For $\beta>0$ and $f\in L^1([0,\Mat])$ the \textbf{fractional integral} of order $\beta$ is
\begin{equation}\label{eq:fracint}
\bigl(I^{\beta}f\bigr)(t) := \frac{1}{\Gamma(\beta)}\int_0^t (t-s)^{\beta-1}f(s)\,\dd s,
\qquad I^0 := \mathrm{Id},
\end{equation}
and for $\beta\in(0,1)$ the \textbf{fractional derivative} of order $\beta$ is
\begin{equation}\label{eq:fracderiv}
\bigl(D^{\beta}f\bigr)(t) := \frac{\dd}{\dd t}\bigl(I^{1-\beta}f\bigr)(t),
\end{equation}
whenever the right-hand side exists.
\end{definition}

For integer $\beta = m$ formula \eqref{eq:fracint} is Cauchy's repeated-integration
formula, so $I^m$ is the $m$-fold iterated integral and $I^1=\int_0^\cdot$; the
extension to real $\beta$ is the unique one preserving the semigroup law
$I^{\beta}I^{\gamma}=I^{\beta+\gamma}$, from which $D^{\beta}I^{\beta}=\mathrm{Id}$
and $D^1 = \dd/\dd t$ follow. Definition \ref{def:fracderiv} is also what appears in
\eqref{eq:roughheston}: the variance is $v = v_0 + I^{\alpha_\Hurst}[\,\cdot\,]$.
Note that \eqref{eq:fracderiv} differentiates $I^{1-\beta}f$ and not $f$, which is why
the initial condition in \eqref{eq:fractionalriccati} is imposed on
$I^{1-\alpha_\Hurst}\psi$; at $\alpha_\Hurst=1$ it reduces to $\psi(u,0)=0$, the
initial condition of \eqref{eq:hestonriccati}.
...
\emph{It is not a theorem of this chapter.} Theorem \ref{thm:roughhestoncf} is proved by techniques
from affine Volterra processes and fractional calculus, and nothing in Sections \ref{sec:roughpaths}
to \ref{sec:signature} enters its proof. This is a division of labour rather than an inconsistency:
at $\Hurst\approx0.1$ the lifting theorems are not available, so a route that does not require them
is necessary rather than merely convenient. What rough path theory supplies here is the vocabulary
--- the regularity exponent, the $p$-variation scale, the precise sense in which the Markov property
has been traded away --- rather than the machinery.
 \end{remark}




\subsection{Rough Geometric Brownian Motion}\label{subsec:roughexample}
...
Fix $\Hurst \in (\tfrac13,\tfrac12]$, $\sigma>0$, $\Spot_0>0$, and $\rf = \divy = 0$, so
that discounting is trivial. Let $\BH$ be a scalar fractional Brownian motion. Also,  for  $\alpha<\Hurst$, let 
$\mathbf{B}^{\Hurst} = (\BH,\lev^{\Hurst}) \in \RPg{\alpha}$  be its
Coutin--Qian lift (Theorem \ref{thm:fbmlift}), and let
$\sigma\mathbf{B}^{\Hurst} := (\sigma\BH,\ \sigma^2\lev^{\Hurst})$ be its dilation, which is again a
geometric rough path by the scaling recorded after Definition \ref{def:rpmetric}. The \textbf{purely rough asset} is the
...
Since the driver is scalar, one ambiguity of the theory is absent here. By \eqref{eq:weaklygeometric} a geometric lift must satisfy
...
comparison with the Itô lift made in Proposition \ref{prop:purelyroughsolution} is a comparison
with a \emph{non}-geometric object.

 \begin{remark}
The vector field $f(y) = \sigma y$ is not
...
large ball and the ball is then exhausted. See \cite{geng2021}. 
\end{remark}
...
\end{equation}
If instead the driver is the Itô lift $\mathbf{W}^{\mathrm{It\hat o}}$ of Example
...
$\mathbf{W}^{\mathrm{It\hat o}}$ with the Itô integral, so \eqref{eq:purelyrough} reads
$\dd\Spot_t = \sigma\Spot_t\,\dd W_t$ and Example \ref{example:blackscholessde} supplies the
solution. The instructive route is the other one: the bracket of the Itô lift is
...
% obtained in Example \ref{example:blackscholessde} by applying Itô's rule to $\log\Spot$. That term
...
% then $\log$ being $\mathcal{C}^2$ on $(0,\infty)$ and $\Spot$ strictly positive, Itô's formula
% would make $\BH = \sigma^{-1}\log(\Spot/\Spot_0)$ a semimartingale too, contradicting Theorem
% \ref{thm:fbmnotsemimartingale}.

% \emph{And therefore the model cannot be traded.} 

Take $\Hurst<\tfrac12$ for the rest of this discussion; the endpoint $\Hurst=\tfrac12$ is excluded
throughout it, and there $\Spot$ \emph{is} a semimartingale and everything below fails.
By \eqref{eq:purelyroughsolution} we have $\BH=\sigma^{-1}\log(\Spot/\Spot_0)$ with $\log$ of class
$\mathcal{C}^{2}$ on $(0,\infty)$, so were $\Spot$ a semimartingale Itô's rule would make $\BH$ one,
contradicting Theorem \ref{thm:fbmnotsemimartingale}. The semimartingale property is preserved under an
...
By the First Fundamental Theorem --- in the general form \cite[Thm.\ 7.2]{delbaenschachermayer1994}
rather than the locally bounded Itô statement of Theorem \ref{thm:firstFTAP} --- the model violates
NFLVR, and Rogers
...
\ref{def:admissible}. Therefore, \textbf{roughness cannot be placed in the price.} It  must then be placed
...
%  A rough price is not a semimartingale, and a
% market whose price is not a semimartingale is not arbitrage-free.  That is exactly the shape of Definition \ref{def:roughheston}: there the
...
What one is entitled to conclude from \eqref{eq:roughexpectedsig} is, however, almost nothing. Example \ref{ex:signatureonedim} showed that the signature of a scalar
...
% Two checks close the discussion.

At $\Hurst=\tfrac12$ the entry reads $\tfrac12\sigma^2\Mat$, which
...
closed form of Fawcett's type is available to us for $\Hurst\neq\tfrac12$: the closed forms of this section stop
...
% geometricity removed the second-order term from the chain rule, and its failure for the Itô lift
...
% a price as illegitimate as \eqref{eq:purelyrough}, and solves it in closed form.
 ...
 \subsection{The optimal execution problem}\label{subsec:execution}

 The First Fundamental Theorem \ref{thm:firstFTAP} is a statement about
...
under the physical measure $\PP$, or under no measure at all, in which no expectation is ever taken: the execution of a large order, the attribution of a hedging profit and
...
One preliminary observation removes the restriction $\Hurst>\tfrac13$ from everything that is
actually \emph{computed} below, even though the definitions that follow continue to state it for the
continuum objects. In practice
...
and Theorem \ref{thm:signatureuniqueness} is what says nothing has been discarded. Stability as the
sampling refines is a separate question --- it needs the lifted paths to converge, hence
$\Hurst>\tfrac13$ again --- and is not claimed here: what is computed is a statistic at a fixed
resolution.
\end{remark}
...
Suppose a trader must liquidate $Q_0>0$ shares over a horizon $[0,\Mat]$ --- hours, not years. Selling fast
...
is a Young integral and integration by parts
is available:
...
it is not a semimartingale, so it
admits no equivalent local martingale measure. Nothing in Definition
...
Proposition \ref{prop:factorialdecay} controls the size of the terms discarded at level $N$. What the
...
\ref{prop:roughexpectedsigaug}. Adjoining $t$ makes \emph{time integration} an
...
$\mathbb{S}:=\Sig^{2N+2}(\widehat\rpX)_{0,\Mat}$ for the truncated signature of the realised
...
The two are proved together, the corollary being the last step of the same computation.

\textbf{\textit{Proof of Proposition \ref{prop:quadform} and Corollary \ref{cor:quadformexpectation}:}}
Take the three terms of \eqref{eq:shortfallreduced} in turn. By Lemma
...
resulting expression is a linear functional of $\mathbb{S}^{2N+2}_\Mat$ whose coefficients are quadratic or
...
with equality only if $\pi\equiv0$ almost surely, which forces $\ell=0$ as soon as the coordinate
functionals $\{\mathbb{S}^{I}\}_{|I|\leq N}$ are linearly independent in $L^{2}$ under the law of
$\widehat X$. That non-degeneracy is assumed here, as it is throughout signature regression; see
\cite{levinlyonsni2013} and \cite{kalsi2020}. \qedfilledtext
...
of method. And it requires the expected signature to level $2N+2$: the terminal inventory is a
functional of words of length $N+1$, and the objective squares it.
...
Further, we take $N=1$ and restrict to the subclass $\ell_{1}=0$, so that the schedule is
...
risen, or slower, according to the sign of $\ell_2$; the omitted term $\ell_{1}t$ is a deterministic
ramp, and we do not claim that it cannot improve the optimum. By Proposition \ref{prop:quadform} the
objective needs the expected signature to level $2N+2=4$. The level-one and level-two entries are
supplied by Proposition \ref{prop:roughexpectedsigaug}, and the entries above them by Proposition
\ref{prop:levelthree} below.

The class \eqref{eq:worked} carries no sign restriction, and it is worth saying so. Since $\pi_t$ is
affine in a Gaussian variable it takes both signs, so the schedule buys as readily as it sells and
the inventory $Q_t$ may pass through zero and go negative. The problem is therefore posed as an
unconstrained quadratic, which is exactly what makes \eqref{eq:optimalell} a linear solve; a genuine
liquidation, with $\pi\ge0$ and $Q\ge0$ imposed, is a constrained programme with no closed form, and
we do not solve it.
...
$\int_0^\Mat G\,\dd\Wt$ is a true Itô integral of mean zero; formula \eqref{eq:levelthree} returns
...
satisfy it. The second check is a genuine one: it uses a structural identity of the
signature to verify a computation performed by Fubini.
\end{remark}
...
% \emph{And the gain is not a scheduling improvement.} 

No pricing measure was used anywhere, and that absence is not incidental: it is the \emph{source}
...
Let $X=(X^{1},X^{2})$ be the two-dimensional path of a pair of assets --- usually
$X^{i}_{t}=\log \left( \Spot^{i}_{t}/\Spot^{i}_{0}\right)$, the cumulative log-return
...
windows it is one half the uncentred covariance of the window returns --- a horizon quantity, not the
intraday realised covariation. Everything else level two contains --- the whole of what
...
the gain of a rule holding, at each instant, an exposure to asset $2$ equal to the move asset $1$
has made since the window opened; and symmetrically for the other term. So $A_{s,t}$ is one half the
difference of two crossed-pairs rules. One qualification keeps the reading honest: with $X^{i}$ the
cumulative \emph{log}-returns, the profit on a unit of asset $2$ is $\dd\Spot^{2}$ and not
$\dd X^{2}$, and the two differ by an It\^o correction. The area is therefore the log-return version
of that trade rather than a figure a desk books directly. What survives the qualification is what
matters here: the rule is non-anticipative and is built from observed prices alone.
\end{remark}

\begin{remark}[Conventions]\label{rem:areaconventions}
The area is insensitive to the Itô--Stratonovich choice. Passing from one to the other adds
...
identical realised covariances but different areas are different rough paths, and they produce
different solutions to a rough differential equation driven by them as soon as its vector fields are
non-constant --- that is, different profit and loss for a path-dependent book. Correlation is not
...
$\Hurst\in(\tfrac13,\tfrac12]$, let $\sigma_{1},\sigma_{2}>0$, $\rho\in[-1,1]$ and $\leadlag\in\R$, and
set
...
\begin{equation}\label{eq:leadlagarea}
  ...
  \;=\;\frac{\sigma_{1}\,\leadlag\,(3-2\Hurst)\,\Mat^{2\Hurst+1}}{4\,(2\Hurst+1)} .
\end{equation}
...
\[
  ...
  +\sigma_{1}\leadlag\!\int_{0}^{\Mat}\!\bigl(\BH_{t}\bigr)^{2}\dd t,
\]
...
\[
  ...
  +\sigma_{1}\leadlag\!\int_{0}^{\Mat}\!G_{t}\,\dd\BH_{t}.
\]
...
\[
  \E\bigl[A_{0,\Mat}\bigr]
  =\frac{\sigma_{1}\leadlag}{2}\,\Mat^{2\Hurst+1}\left(\frac{1}{2\Hurst+1}-\frac{2\Hurst-1}{2(2\Hurst+1)}\right)
  =\frac{\sigma_{1}\leadlag\,\Mat^{2\Hurst+1}}{2(2\Hurst+1)}\cdot\frac{3-2\Hurst}{2},
\]
which is \eqref{eq:leadlagarea}. \qedfilledtext

\medskip

At $\Hurst=\tfrac12$ the expression reduces to $\sigma_{1}\leadlag\Mat^{2}/4$, which is checked
directly: there $\int G\,\dd\Wt$ is an Itô integral of mean zero and only the first term survives.
At $\leadlag=0$ it vanishes for every $\rho$, as it must, the L\'evy area of a correlated Brownian pair
having mean zero.
...
a pair in which the contemporaneous coupling $\rho$ and the lagged coupling $\leadlag$ are separately
adjustable, and shows that the area sees the second and is \emph{exactly blind} to the first in
expectation --- its variance still carries $\rho$, through the factor $\sqrt{1-\rho^{2}}$ on the two
mixed terms of the proof, so it is the mean and not the law that is free of it. A
...
\[
  \widehat\leadlag\;=\;\frac{4(2\Hurst+1)\,\widehat{\E}\bigl[A_{0,\Mat}\bigr]}
...
% \eqref{eq:roughvolintegral} exists as an ordinary Itô integral by Section
...
% \begin{itemize}
%   \item[i)] \emph{A pathwise construction.} The Itô integral is an $L^{2}$ limit defined off a null
...
%     than in the construction of the model, that the continuity of the Itô--Lyons map, Theorem
...
\noindent
Rough path theory is Lyons' \cite{lyons1998}, and the Hölder presentation followed here is that of
Friz and Hairer \cite{frizhairer2020}, which is the natural companion to this chapter and the source
of every proof quoted. The original $p$-variation formulation, together with the
free nilpotent groups that the general case requires and which Definition \ref{def:tensoralgebra}
was truncated to avoid, is developed systematically in Friz and Victoir \cite{frizvictoir2010}; the
lecture notes of Geng \cite{geng2021} are a shorter route to the same material. Three attributions
made in passing above deserve their own reference. The multiplicative property \eqref{eq:chenaxiom}
is Chen's \cite{chen1957}, where the iterated integrals of a smooth path were first organised into a
group. The sewing lemma, the controlled paths of Definition \ref{def:controlled} and the integral of
Theorem \ref{thm:gubinelli} are Gubinelli's \cite{gubinelli2004}, and it is that paper which put the
two-level theory into the form used here. The pathwise integral that Subsection
\ref{subsec:whatfails} finds out of reach is Young's \cite{young1936}.

\medskip

\noindent
On the fractional side, the process of Definition \ref{def:fbm} goes back to Kolmogorov and takes its
modern form in Mandelbrot and Van Ness \cite{mandelbrotvanness1968}; the Gaussian facts collected in
Proposition \ref{prop:fbmproperties}, including the negative half of the Hölder statement, are in
Marcus and Rosen \cite[Ch.\ 7]{marcusrosen2006}. Theorem \ref{thm:fbmnotsemimartingale} is Rogers'
\cite{rogers1997}, which also exhibits the arbitrage that Subsection \ref{subsec:roughexample} appeals
to.  The
canonical lift of Theorem \ref{thm:fbmlift} is Coutin and Qian's \cite{coutinqian2002}. It is worth
recording what lies immediately outside it. The
piecewise-linear lifts converge only for $\Hurst>\tfrac14$, and at no number of levels below that;
the Lyons--Victoir extension theorem \cite{lyonsvictoir2007} nevertheless guarantees, by a
Hahn--Banach argument, that \emph{every} $\alpha$-Hölder path admits some lift, at the cost of the
axiom of choice and with no canonical choice among them. 
\medskip

\noindent
The signature of Section \ref{sec:signature} is Chen's construction again, read as a transform.
Uniqueness for paths of bounded variation is Hambly and Lyons \cite{hamblylyons2010}, and its
extension to rough paths is Boedihardjo, Geng, Lyons and Yang \cite{boedihardjo2016}; the
expected-signature statement of Theorem \ref{thm:expectedsignature} is Chevyrev and Lyons
\cite{chevyrevlyons2016}. Its use as a feature map for learning on path space, which is what
Remark \ref{rem:signaturecalibration} is about, is surveyed in \cite{chevyrevkormilitzin2016} and
\cite{levinlyonsni2013}. The execution problem of Subsection \ref{subsec:execution} is Almgren and
Chriss' \cite{almgrenchriss2001}, of which \cite{carteajaimungal2015} is the textbook account; its
signature formulation, of which Proposition \ref{prop:quadform} and the worked instance
\eqref{eq:worked} are the scalar case, is Kalsi, Lyons and Perez Arribas \cite{kalsi2020}. The same
reduction --- an objective built from time integrals, integrals against the price and polynomials of
the control becomes a quadratic form in one expected signature --- underlies the non-parametric
pricing and hedging of path-dependent claims in \cite{lyonsnejadperez2020}, the path-dependent
processes of \cite{abijaber2024}, and the signature market models calibrated jointly to SPX and VIX
in \cite{cuchiero2025}.

\medskip

\noindent
The empirical finding of Subsection \ref{subsec:roughvol} is Gatheral, Jaisson and Rosenbaum
\cite{gatheraljaissonrosenbaum2018}, which inverts the long-memory reading of Comte and Renault
\cite{comterenault1998}; the literature it opened is surveyed in \cite{bayerfrizfukasawa2023} and
\cite{dinunno2023}. Theorem \ref{thm:roughhestoncf} is El Euch and Rosenbaum's
\cite{eleucrosenbaum2019}. Remark \ref{rem:roughhestoncost} was explicit that no result of Sections
\ref{sec:roughpaths} to \ref{sec:signature} enters its proof, and the reason is a necessity rather
than an oversight: at the exponent $\Hurst\approx0.1$ that the data report, the lifting theorems are
not available, so the route through affine Volterra processes and fractional calculus is the only
one open. What does reach that regime is Hairer's theory of regularity structures
\cite{hairer2014}, of which rough path theory is the one-dimensional special case, and the structure
built there for rough volatility is \cite{bayerfrizgassiat2020}. Neither is developed here; both are
recorded to mark the edge of what has been proved.
...
% \cite{dinunno2023}. Theorem \ref{thm:roughhestoncf} is El Euch and Rosenbaum's
% \cite{eleucrosenbaum2019}. Remark \ref{rem:roughhestoncost} was explicit that no result of Sections
% \ref{sec:roughpaths} to \ref{sec:signature} enters its proof, and the paragraph above says why this
% is a necessity rather than an oversight: at the exponent $\Hurst\approx0.1$ that the data report, the
% lifting theorems are simply not available, so a route through affine Volterra processes and
% fractional calculus is the only one open. What does reach that regime is Hairer's theory of regularity
% structures \cite{hairer2014}, of which rough path theory is the one-dimensional special case, and in
% which the fixed local expansion \eqref{eq:controlleddef} is replaced by an expansion in an abstract
% basis of symbols and the products that fail to converge are repaired by renormalisation. The
% structure built for rough volatility is \cite{bayerfrizgassiat2020}, where the difficulty is
% localised, by the conditioning device of Romano and Touzi \cite{romanotouzi1997}, in the single
% leverage term that the correlation $\rho$ multiplies --- so that the feature which makes a rough
% volatility model worth having, namely the skew of Subsection \ref{subsec:smile_skew}, is precisely
% the feature that puts it beyond the integration theory of Chapter \ref{ch:foundations}. The general
% account is \cite[Ch.\ 13--14]{frizhairer2020}. None of it is
% developed here; it is recorded to mark honestly the edge of what has been proved.
