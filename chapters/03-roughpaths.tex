...
in place of a pathwise one, and a definition valid only up to null sets. 
...
direction. Only the balance point $\Hurst = \tfrac12$ escapes. The consequence is severe and complete: for $\Hurst \neq \tfrac12$ there is no Itô integral
against $\BH$, no Itô formula, no martingale representation, and no Girsanov theorem.
...
factor produces a skew that flattens as $\tau \downarrow 0$, and no choice of
