...
% \begin{convention}[The symbol $\blacksquare$]\label{conv:blacksquare}
% A result whose statement ends with $\blacksquare$ is quoted \textbf{without proof}, a reference
% being given in every case. A result without the symbol is one we prove, either immediately below it
% or --- when the argument is routine but too long for the body --- in Appendix \ref{app:proofs}. The
% convention is followed throughout Part \ref{part:part1} and Part \ref{part:part2}.
% \end{convention}
...
Property \textit{i)} is what guarantees that $X^2$ is a submartingale whenever $X$ is a martingale, a fact invoked twice in what follows, in Subsection \ref{subsec:martingales} and again in the Doob-Meyer decomposition, and it is the reason convexity is such a persistent theme in option pricing.
...
Completeness is what allows us to modify a process on a null set without leaving the filtration, as we shall do whenever we choose a continuous version of an object defined only up to indistinguishability. Right-continuity is what makes first hitting times of open sets stopping times, and it is used explicitly in Proposition \ref{prop:stoppingtimechar} below and again in the Doob-Meyer decomposition of Subsection \ref{subsec:measurechanges}. 
...
\begin{example}\label{example_stoppingTimes}
    ...
    whereas $\{\tau_n < 0\} = \emptyset \in \mathcal{F}_0$. By Proposition \ref{prop:stoppingtimechar}, $\tau_n$ is a stopping time.
     ...
     It is exactly the localising sequence that will allow us, in Subsection \ref{subsec:itointegral}, to define the stochastic integral for integrands that are square-integrable pathwise but not in expectation, and it is the prototype of every localisation argument in this chapter.
\end{example}
...
\begin{itemize}
  ...
  \item Conversely, suppose $\charf_{X_n}(u) \to \charf(u)$ for every $u \in \R$, where $\charf : \R\to\Cx$ is \emph{some} function continuous at $u=0$. Then $\charf$ is the characteristic function of a random variable $X$, and $X_n \xrightarrow{d} X$. \hfill $\blacksquare$
\end{itemize}
\end{theorem}

 In the converse direction one is \emph{not} allowed to assume that the limit $\charf$ is a characteristic function; that is the conclusion, not a hypothesis. Continuity at the origin is what rules out mass escaping to infinity, and it is a genuine restriction on $\charf$; note that if one already knew $\charf$ to be a characteristic function the requirement would be vacuous, since characteristic functions are automatically uniformly continuous. It is precisely this form of the statement that makes the Central Limit Theorem below a two-line computation.
...
Martingales are processes for which the expected value of a future instant is precisely the state of the process up to present time. Martingales are absolutely key in the study of stochastic processes and some generalizations are needed as well.
...
than its conclusion. Two remarks on the hypotheses before we use the theorem. They are not independent: iii) implies ii), 
...
Two cautions are in order. First, the limit in \eqref{eq:qvdef} need not exist for an arbitrary $f$, which is why the definition is phrased conditionally. Second, for a \emph{stochastic} process the sums in \eqref{eq:qvdef} are random variables, and the limit must be specified as a limit of random variables. We shall always understand it in $L^2(\Omega)$, which for our purposes is both the easiest to obtain and strong enough to imply convergence in probability, and hence almost sure convergence along a subsequence.

It is worth pausing on how strange \eqref{eq:qvdef} is from the point of view of ordinary calculus. If $f$ is continuously differentiable, then $|f(t_k)-f(t_{k-1})| \leq C\,(t_k - t_{k-1})$ and the sum is bounded by $C^2 \lVert\mathcal{P}\rVert \, t \to 0$: smooth functions have zero quadratic variation. The same computation applies to any continuous function of bounded variation. A non-zero quadratic variation is therefore a certificate of irregularity, and Proposition \ref{prop:bm-properties}(e) below will assert that Brownian motion carries exactly such a certificate.

For a martingale there is a second and less elementary route to the same object, which we state here
in advance because the notation is needed almost immediately. We shall see in Theorem
\ref{thm:doobmeyer} that if $M$ is a square-integrable martingale, then $M^2$ admits a unique
decomposition
\begin{equation}\label{eq:compensatorannounce}
M_t^2 = R_t + \qv{M}_t ,
\end{equation}
in which $R$ is a martingale and $\qv{M}$ is predictable, increasing and null at the origin. The
process $\qv{M}$ is called the \textbf{compensator} of $M^2$, or the \emph{predictable quadratic
variation} of $M$. Proposition \ref{prop:qvcoincide} will show that for continuous $M$ it coincides
with the pathwise $[M]$ of Definition \ref{def:quadratic-variation}, so that no ambiguity arises
from writing $\qv{M}$ throughout, as we shall. Nothing in the present subsection depends on the
proof of either statement; only the notation is being borrowed.
...
X_t = M_t + A_t,
\]
where $M_t$ is a local martingale and $A_t$ is adapted,  \textit{continue à droite, avec limites à gauche} (càdlàg), of locally bounded variation and $A_0$ = 0.
\end{definition}
...
set of requirements in Definition~\ref{def:Brownian-motion}. We have demanded independent Gaussian increments \emph{and} continuity of every sample path, 
...
so we indicate the results which settle it.
...
and we shall be careful to say which of the two we mean.

That the theorem delivers only the weaker of the two is not a defect but a necessity, since
\eqref{eq:kolmogorovmoment} constrains only two-dimensional distributions and cannot possibly
determine path behaviour on its own. What matters for the construction at hand is the other side of
the coin: a modification has, by definition, the same finite-dimensional distributions as the
process it modifies. As is customary, we shall always work with the continuous modification and
...
the modifications so obtained agreeing almost surely on overlaps. Existence is settled.
...
We can now state the promised return to the simple symmetric random walk of Example
\ref{example:randomwalk}, where we said we would come back to it once the jump size and the waiting
time between jumps had been adjusted together.
...
\subsubsection*{The reflection principle and the running maximum}
...
$\mathcal{F}_\tau$ (Definition \ref{def:Ftau}). \hfill $\blacksquare$
\end{theorem}

The statement genuinely strengthens Proposition \ref{prop:bm-properties}(d), where the Markov
property was asserted at deterministic times only, and it is not automatic: it holds here because
$\tau$ may be approximated from above by stopping times taking countably many values, for each of
which the claim reduces to the ordinary Markov property, and the continuity of the paths lets one
pass to the limit. See \cite[Thm.\ 2.6.16]{karatzasshreve1991}.

Fix $a>0$ and let $\tau_a := \inf\{t \geq 0 : W_t > a\}$ be the first hitting time of the level $a$,
...
\textit{Sketch.} The first chain of equalities is the argument just given: $\{\overline{W}_t \geq a\} = \{\tau_a \leq t\}$
...
differentiates in both arguments; see \cite[Sec.\ 2.6]{karatzasshreve1991} or \cite{shreve2004}.
...
model and are precisely the contracts for which the local volatility model of Subsection
\ref{subsec:localvol}, calibrated to terminal marginals alone, gets the answer wrong. We return to
...
Finally, we can make good on the announcement of Subsection \ref{subsec:martingales} and record the
precise relation between the pathwise quadratic variation of Definition \ref{def:quadratic-variation}
and the predictable version \eqref{eq:compensatorannounce} produced by the Doob--Meyer decomposition
of Subsection \ref{subsec:measurechanges}, since both notations occur in the literature and we shall
...
\quad X_0 \in \R.
\end{equation}
Here $dW_t$ will be specified in the next subsection. Notice that this generalizes the deterministic case: if $b \equiv 0$, then $X_t$ reduces to the solution of the ODE $dY_t = a(t,Y_t)\,dt$. In a sense, instead of modeling a proper trajectory, a ''position" as a function of time, we will model a probability distribution per time. This makes much more sense in many applications, such as finance and quantum mechanics.
...
It must be highlighted that the following dissertation can be expanded up to the most general setting anticipated with semimartingales (Definition \ref{def:semimartingale}), however we will stick to the most classical presentation of the subject, leaving the most general case a worthy space at the end, but without letting the trees hide the forest.
...
Let ($\Omega, \mathcal F, \{\mathcal F_t\}, \PP$) be a filtered probability space.
\begin{itemize}
...
\begin{example}\label{example:blackscholessde}
    In Finance, the Black-Scholes model relies on the \textit{log-normal} equation, given by $dS_t = rS_t \, dt + \sigma S_t \, dW_t$, where $r, \sigma \in \R$ and $\sigma > 0$. It can be solved by taking $H_t = \ln (S_t),$ so that
    \begin{equation}
...
The modulus for the diffusion coefficient is written $\eta$ rather than the $h$ of the original
paper, since $h$ denotes the terminal payoff function in Theorem \ref{thm:feynmankac}.
...
Definition \ref{def:correlatedbm} below, \eqref{eq:itomulti} is unchanged and
...
\begin{theorem} \label{theorem:solutions:ndimensional_sde}
    ...
    $$ 
    where $\lVert \sigma \rVert^2 = \sum_i \sum_j \sigma_{ij}^2$. For any $\mathcal{F}_0$-measurable random vector $X_0$ of finite variance, the SDE $dX_t = \mu(t,X_t) \, dt +  \sigma(t,X_t) \, dW_t  $, $X_0 = \xi$ has a unique continuous solution. \hfill $\blacksquare$
\end{theorem}
...
\begin{theorem}[Doob--Meyer Decomposition Theorem]\label{thm:doobmeyer}
    ...
    \begin{equation}
        M_t ^2 = R(t) + \qv{M}_t
    \end{equation}
    where $R(t)$ is \textit{càdlàg} martingale and $\qv{M}_t$ is a predictable, right continuous and increasing process such that $\qv{M}_0 = 0$ and $\E\bigl[\qv{M}_t\bigr] < \infty$ for all $t\in[0,T].$ We call $\qv{M}_t$ the \textbf{compensator} of $M_t^2$ and we say it is the predictable quadratic variation of $M_t$. \hfill $\blacksquare$
\end{theorem}
...
\begin{theorem}[Novikov's condition]\label{thm:novikov}
    Let $\alpha \in \mathcal L (\Omega, L^2[0,T])$. If $\E\left[ e^{\frac 1 2 \int_0^T \alpha^2(s) ds} \right] < \infty$, then $\mathcal E_\alpha(t)$ is a martingale. A proof may be found in \cite[Sec.~8.7]{kuo2006}. \hfill $\blacksquare$
...
\begin{lemma}[Conditional expectation under a change of measure]\label{lem:condexpchange}
    Let $\gamma$ be a nonnegative integrable random variable, so that $d\QQ = \gamma\, d \PP$ defines a new probability measure (see \ref{eq:radonnikodym}). Then, for any $\sigma$-algebra $\mathcal{G} \subset \mathcal{F}$ and $Y$ such that $\E_{\QQ}[Y] < \infty$, it follows that
    \begin{equation}
...
One hypothesis is being used silently here and is worth naming, since it is a natural examiner's
question. Theorem \ref{thm:mrt} is applied under $\QQ$ to the $\QQ$-Brownian motion $\mathbf{Z}$, so
...
\begin{definition}[Diffusion Process]\label{def:diffusion}
    An $\R^n$-valued process $X_t$, $t\in [0,T]$ is called a \textbf{diffusion process} if its transition probabilities $\{P_{s,x}(t, \cdot )\}$ satisfy the following three conditions for every $t\in [0,T]$, every $x\in\R^n$ and every $\delta > 0$:
...
\begin{theorem}\label{thm:sdeisdiffusion}
    Let $\sigma(t,x)$ and $\mu(t,x)$ be functions as in Theorem \ref{theorem:solutions:ndimensional_sde}. Assume they are also continuous on $[0,T] \times \R^n$. The solution to the SDE there defined is a diffusion process where $\mu(t,x)$ is identified and $\Sigma(t,x)= \sigma(t,x)\,\sigma(t,x)\transp$. \hfill $\blacksquare$
\end{theorem}
...
\begin{equation}
    \mathcal{K} = \{ (H \cdot X)_T \mid H \text{ is admissible and } V_0(H) = 0 \}
\end{equation}
...
The market satisfies \textbf{NFLVR} if:
\begin{equation}
...
Assume the process $X$ is locally bounded. The market satisfies the NFLVR condition if and only if there exists at least one Equivalent Local Martingale Measure $\QQ$. \hfill $\blacksquare$
\end{theorem}
...
Assume that the set of equivalent local martingale measures $\mathcal{M}^e$ is non-empty (i.e., the market is arbitrage-free). The market is complete if and only if $\mathcal{M}^e$ is a singleton set (i.e., $\QQ$ is unique). \hfill $\blacksquare$
\end{theorem}
...
\begin{equation}
    \frac{Z_t}{S^0_t} = \Ep{\QQ^S}\left[ \frac{Z_T}{S^0_T} \mid \mathcal{F}_t \right] \quad \text{and} \quad \frac{Z_t}{N_t} = \Ep{\QQ^N}\left[ \frac{Z_T}{N_T} \mid \mathcal{F}_t \right]
\end{equation}
...
\begin{theorem}[Feynman--Kac Formula]\label{thm:feynmankac}
    ...
    \end{equation}
    with initial condition $X_t = x$, where $W$ is a standard $n$-dimensional $\QQ$-Brownian motion, and the diffusion matrix is given by $\Sigma(s, x) = \sigma(s,x)\sigma(s,x)\transp$. Let $h: \R^n \rightarrow \R$ be a Borel-measurable terminal payoff function, and $\rf: [0,T] \times \R^n \rightarrow \R$ be a continuous function representing the continuously compounded interest rate. (The time variable is written $s$, and not $\tau$ as would be natural, because $\tau$ is reserved for stopping times and for the time to maturity $T-t$.)
   ...
   Assume $u(t,x)$ is a sufficiently regular function (typically $u \in C^{1,2}([0,T) \times \R^n) \cap C([0,T] \times \R^n)$) that satisfies the following linear parabolic partial differential equation:
    ...
    \begin{equation}\label{FK_Expectation}
        u(t,x) = \Ep{\QQ} \left[ \left. e^{-\int_t^T \rf(s, X_s) ds} h(X_T) \right| X_t = x \right].
    \end{equation}
    \hfill $\blacksquare$
\end{theorem}
...
emphatically not the physical $\PP$; writing a bare $\E$ there would silently conflate the two
measures that the whole of Subsection \ref{subsec:bs_model} is at pains to keep apart. We therefore
name it here.
...
\vartheta &:= \frac{\partial C}{\partial t} = -\frac{\Spot\,N'(d_+)\sigma}{2\sqrt{\tau}} - \rf \Strike e^{-\rf\tau}N(d_-). \label{eq:vegatheta}
\end{align}
\hfill $\blacksquare$
\end{proposition}

The names are delta, gamma, vega and theta respectively. Two notational cautions: vega is not a
Greek letter and has no standard symbol, so we write $\mathcal{V}$; and theta is written $\vartheta$
rather than $\Theta$, the capital being reserved throughout for the spot-to-forward conversion
factor $\Carry$ of \eqref{eq:spotforwardfactor} and for nothing else.

Two identities among these are worth more than the formulae themselves. The first is that
...
precisely because the payoff is convex, and the reason Proposition \ref{prop:condjensenl2}, item i),
has been such a persistent theme.

The second is that substituting \eqref{eq:deltagamma} into the Black--Scholes equation
\eqref{eq:BS_PDE} rearranges it into
\begin{equation}\label{eq:thetagamma}
\vartheta + \tfrac12\sigma^2\Spot^2\,\Gamma \;=\; \rf\bigl(C - \Spot\,\Delta\bigr),
\end{equation}
which says that for a delta-hedged position ($\Delta$ neutralised, so that the right-hand side is
just the financing of the residual cash) the time decay $\vartheta$ and the convexity
...
The Local Volatility (LV) framework, introduced by Dupire \cite{dupire1994}, posits that volatility is a deterministic function of the current asset price and time: $\lvol = \lvol(\Spot_t, t)$. The dynamics under the risk-neutral measure are:
\begin{equation}\label{eq:localvolspot}
    d\Spot_t = (\rf-\divy)\,\Spot_t\,dt + \lvol(\Spot_t, t)\,\Spot_t\,d\Wt_t
\end{equation}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
where $\mu_v$ and $\sigma_v$ are written thus, rather than as the $\alpha$ and $\beta$ common in the
literature, because both those letters are already spoken for --- $\alpha$ as the Girsanov integrand
of Subsection \ref{subsec:measurechanges} and as the Hölder exponent throughout Chapter
\ref{ch:roughpaths}, and $\alpha,\beta$ jointly as the exponents of Kolmogorov's criterion
\eqref{eq:kolmogorovmoment}. A critical parameter is the correlation $\rho$ between the two Brownian motions: $d\langle \Wt^{\Spot}, \Wt^{v} \rangle_t = \rho\,dt$.
\begin{itemize}
...
\begin{itemize}
    ...
    $\mathfrak{C} = \{(t,\omega) : \Snell_t > Y_t\}$, the fraktur letter distinguishing it from the
    cone $\mathcal{C}$ of dominated claims in Subsection \ref{subsec:derivativepricing};
\end{itemize}
