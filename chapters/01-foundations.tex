...
$\sigma$-algebra of events, and $\mathbb{P}$ a probability measure. A \textbf{random variable} is a
measurable map $X : (\Omega,\mathcal{F}) \to (E,\mathcal{E})$, the target being
$(\mathbb{R},\mathcal{B}(\mathbb{R}))$ in most of our applications.

\begin{convention}
Every probability space in this dissertation is assumed \emph{complete}: if $A \in \mathcal{F}$ has
$\mathbb{P}(A)=0$, then every $B \subseteq A$ lies in $\mathcal{F}$ and has $\mathbb{P}(B) = 0$.
\end{convention}

Completeness is not a formality. It is what will allow us, throughout, to modify a process on a null
set --- as we must whenever a theorem produces ``a continuous version'' of an object defined only up
to indistinguishability --- without leaving the space we started in. This is the first of several
points at which the measure-theoretic language earns its place: it is not a technical convenience but
the only setting in which path regularity, filtrations and martingale properties can be discussed at
all.
...
it satisfies the tower property $\mathbb{E}[X\mid\mathcal{H}] =
\mathbb{E}[\mathbb{E}[X\mid\mathcal{G}]\mid\mathcal{H}]$ for $\mathcal{H}\subseteq\mathcal{G}$. These,
...
\begin{proposition}[Conditional Jensen and the $L^2$ projection]\label{def:conditional-expectation}
...
\begin{itemize}
    \item For $a, b \in \mathbb{R}$, $aX + bY \sim \mathcal{N}(a\mu_X+b\mu_Y,\, a^2\sigma_X^2+b^2 \sigma_Y^2)$; if instead $X$ and $Y$ are jointly normal with correlation $\rho$, the variance becomes $a^2\sigma_X^2+b^2\sigma_Y^2+2ab\rho\,\sigma_X\sigma_Y$. We write $Z \sim \mathcal{N}(\mu,0)$ for the constant variable $\mu$, so that the identity persists when constants are combined with normal variables.
    \item Its characteristic function is entire analytic, and $X_n \sim \mathcal{N}(\mu_n,\sigma_n^2)$ converges in distribution to $\mathcal{N}(\mu,\sigma^2)$ with $\sigma>0$ if and only if $\mu_n\to\mu$ and $\sigma_n^2\to\sigma^2$.
...
noises are always Gaussian several variables at a time.

\begin{example}[Multivariate Normal Distribution]\label{example:multivariatenormal}
An $\mathbb{R}^d$-valued random vector $X$ follows a \textbf{multivariate normal distribution} with
mean $\mu \in \mathbb{R}^d$ and covariance $\Sigma \in \mathbb{R}^{d\times d}$, written
$X \sim \mathcal{N}_d(\mu,\Sigma)$, if $a^\top X \sim \mathcal{N}(a^\top\mu,\, a^\top\Sigma a)$ for
every $a \in \mathbb{R}^d$.
\end{example}

Here $\Sigma$ is necessarily symmetric positive semi-definite. If it is invertible the law is
absolutely continuous on $\mathbb{R}^d$ with density
...
All positive moments exist and are given by $\mathbb{E}[X^k] = \exp(k\mu + \tfrac12 k^2\sigma^2)$; in
particular $\mathbb{E}[X] = e^{\mu+\sigma^2/2}$ and
$\mathbb{V}[X] = (e^{\sigma^2}-1)e^{2\mu+\sigma^2}$. All real moments therefore exist, and yet the
...
i) Let $\{\xi_n\}$ be i.i.d.\ with $\mathbb{P}(\xi_n = 1) = \mathbb{P}(\xi_n=-1)=\tfrac12$. The
...
ii) Fix $\lambda>0$. A process $N = \{N_t\}_{t\geq0}$ with values in $\mathbb{N}_0$ is a
\textbf{Poisson process with rate $\lambda$} if $N_0 = 0$ almost surely, $N$ has independent
increments, and
\begin{equation}
\mathbb{P}(N_{t+s}-N_s = k) = \frac{(\lambda t)^k}{k!}e^{-\lambda t}, \qquad k \in \mathbb{N}_0,\ s,t\geq0 .
\end{equation}
Its paths are piecewise constant and non-decreasing, with unit jumps at random times;
$\mathbb{E}[N_t] = \mathbb{V}[N_t] = \lambda t$ and
...
$\mathbb{T} = [0,\infty)$, while having discontinuous sample paths --- a distinction worth keeping
...
The \textbf{transition probability} of a Markov process is $\mathbb{P}(X_t \in A \mid X_s = x)$; in order to integrate against the measure it defines we write $P_{s,x}(t,dy) = \mathbb{P}(X_t \in dy \mid X_s = x).$
...
\[
\mathbb{E}\Bigl[\prod_{k=1}^n g_k(\Delta_k)\Bigr]
= \mathbb{E}\bigl[g_n(\Delta_n)\bigr]\;\mathbb{E}\Bigl[\prod_{k=1}^{n-1} g_k(\Delta_k)\Bigr]
= \prod_{k=1}^n \mathbb{E}\bigl[g_k(\Delta_k)\bigr],
\]
...
Therefore, a process \(W(t)\) satisfying the conditions of 
...
\(W_t\) is \(\mathcal{F}_t\)-measurable and increments \(W(t)-W(s)\) are independent 
...
\textit{\textbf{Proof}.} a) By i) we have $W_t = W_t - W_0$ almost surely, so $W_t \sim \mathcal{N}(0,t)$
by iii). For $s < t$, write $W_t = (W_t - W_s) + W_s$ and use that $W_t - W_s$ is independent of
$W_s$ with zero mean:
\[
\mathbb{E}[W_sW_t] = \mathbb{E}\bigl[W_s(W_t-W_s)\bigr] + \mathbb{E}[W_s^2] = 0 + s = \min\{s,t\}.
\]

b) $Y_0 = 0$ and the paths of $Y$ are continuous, since those of $W$ are. For $s<t$ we have
$Y_t - Y_s = W_{t+t_0} - W_{s+t_0} \sim \mathcal{N}(0,t-s)$, and increments of $Y$ over disjoint
...
\[
\mathbb{E}[W_t \mid \mathcal{F}_s] = \mathbb{E}[W_t - W_s \mid \mathcal{F}_s] + \mathbb{E}[W_s\mid\mathcal{F}_s]
= \mathbb{E}[W_t-W_s] + W_s = W_s,
\]
so $W$ is a martingale; the same computation with $\mathbb{P}(W_t \in A \mid \mathcal{F}_s)$ in place
of $\mathbb{E}[W_t\mid\mathcal{F}_s]$ shows that the conditional law of $W_t$ given $\mathcal{F}_s$
...
$\mathbb{E}[X_k]=0$, and since $\mathbb{E}[Z^4]=3\sigma^4$ for $Z\sim\mathcal{N}(0,\sigma^2)$,
\[
\mathbb{E}[X_k^2] = 3(t_k-t_{k-1})^2 - 2(t_k-t_{k-1})^2 + (t_k-t_{k-1})^2 = 2(t_k-t_{k-1})^2 .
\]
Hence, by independence and orthogonality,
\[
\mathbb{E}\Bigl[\Bigl(\sum_k (W_{t_k}-W_{t_{k-1}})^2 - t\Bigr)^{\!2}\Bigr]
= \sum_k \mathbb{E}[X_k^2] = 2\sum_k (t_k-t_{k-1})^2 \leq 2\lVert\mathcal{P}\rVert\,t \longrightarrow 0
\]
...
and conditioning,
\[
\mathbb{E}[W_t^2 \mid \mathcal{F}_s] = (t-s) + 2W_s\cdot 0 + W_s^2,
\]
so $\mathbb{E}[W_t^2 - t \mid \mathcal{F}_s] = W_s^2 - s$. \hfill $\blacksquare$
...
\[
\mathbb{E}\bigl[\varphi(t_{i-1})\Delta_i W\bigr]
= \mathbb{E}\Bigl[\varphi(t_{i-1})\,\mathbb{E}\bigl[\Delta_i W \mid \mathcal{F}_{t_{i-1}}\bigr]\Bigr] = 0,
\]
and summing yields $\mathbb{E}[I(f)]=0$. For the second moment, expand the square. If $i<j$ then
...
\[
\mathbb{E}\bigl[\varphi(t_{i-1})^2 (\Delta_i W)^2\bigr]
= \mathbb{E}\bigl[\varphi(t_{i-1})^2\bigr](t_i - t_{i-1}) = \int_{t_{i-1}}^{t_i}\mathbb{E}[f(t)^2]\,dt,
\]
since $f \equiv \varphi(t_{i-1})$ on $[t_{i-1},t_i)$. Summing over $i$ gives
$\mathbb{E}[I(f)^2] = \int_0^T \mathbb{E}[f(t)^2]\,dt$. \hfill $\blacksquare$
...
\[
\mathbb{E}\bigl[(I(f_n)-I(f_m))^2\bigr] = \int_0^T \mathbb{E}\bigl[(f_n(t)-f_m(t))^2\bigr]\,dt
\xrightarrow[n,m\to\infty]{} 0,
\]
so $\{I(f_n)\}$ is Cauchy in $L^2(\Omega)$ and its limit $I(f)$ exists and is independent of the
approximating sequence. Both $f \mapsto \mathbb{E}[I(f)]$ and $f \mapsto \mathbb{E}[I(f)^2]$ are
continuous for the $L^2$ norms involved, so the identities $\mathbb{E}[I(f_n)]=0$ and
$\mathbb{E}[I(f_n)^2] = \int_0^T\mathbb{E}[f_n^2]\,dt$ pass to the limit.

ii) Apply i) to $f+g$ and expand:
$\mathbb{E}[I(f+g)^2] = \mathbb{E}[I(f)^2] + 2\mathbb{E}[I(f)I(g)] + \mathbb{E}[I(g)^2]$ by linearity
...
\noindent In the language of Proposition \ref{def:conditional-expectation}, item 9, this is precisely
...
$\mathbb{E}[X_t^2] = \int_0^t \mathbb{E}[f(s)^2]\,ds < \infty$ and $L^2(\Omega) \subset L^1(\Omega)$
...
\[
\mathbb{E}\bigl[\varphi(t_{i-1})(W(t_i)-W(t_{i-1})) \mid \mathcal{F}_s\bigr]
= \mathbb{E}\Bigl[\varphi(t_{i-1})\,\mathbb{E}\bigl[W(t_i)-W(t_{i-1}) \mid \mathcal{F}_{t_{i-1}}\bigr] \,\Big|\, \mathcal{F}_s\Bigr] = 0,
\]
by the tower property (Proposition \ref{def:conditional-expectation}, item 5) and independence of the
increment from $\mathcal{F}_{t_{i-1}}$. Hence $\mathbb{E}[X_t \mid \mathcal{F}_s] = X_s$.
...
conditional expectation is a contraction on $L^1$ (Proposition \ref{def:conditional-expectation},
item 7), so $\mathbb{E}[X^n_t \mid \mathcal{F}_s] \to \mathbb{E}[X_t \mid \mathcal{F}_s]$ in
...
\noindent The continuity hypothesis on $\mathbb{E}[f(t)f(s)]$ is what makes this work and cannot
...
that $\mathbb{E}[Z_t^2]$ is finite, and an inequality between infinite quantities would tell us
nothing. Both obstructions are removed by the same device. Define, for $n \in \mathbb{N}$,
...
\[
\mathbb{E}\left[\left(\int_0^{t\wedge\tau_n} \bigl[\sigma(s,X_s)-\sigma(s,Y_s)\bigr]dW_s\right)^{\!2}\right]
= \int_0^{t} \mathbb{E}\bigl[(\sigma(s,X_s)-\sigma(s,Y_s))^2\,\mathds{1}_{\{s\leq\tau_n\}}\bigr]ds
\leq K^2\!\int_0^t \mathbb{E}\bigl[Z_s^2\,\mathds{1}_{\{s\leq\tau_n\}}\bigr]ds .
\]
...
\]
Write $u_n(t) := \mathbb{E}\bigl[Z_{t\wedge\tau_n}^2\bigr]$. Since $|Z_{t\wedge\tau_n}| \leq 2n$ we
...
event on which $Z$ has continuous paths. Then $\mathbb{P}(\Omega_0)=1$, and for $\omega \in \Omega_0$
...
imposed for the solution to make sense, and Itô's formula holds for $\theta(X_t)$ as before, now
accounting for the crossed second derivatives $\partial^2\theta/\partial x_l \partial x_h$.
...
$\mathbb{E}[\mathcal{E}_\alpha(t)] = \mathbb{E}\bigl[\mathbb{E}[\mathcal{E}_\alpha(t)\mid\mathcal{F}_0]\bigr]
= \mathbb{E}[\mathcal{E}_\alpha(0)] = 1$ for every $t$.
...
$s<t$,
\[
\mathbb{E}\bigl[\mathcal{E}_\alpha(t) \mid \mathcal{F}_s\bigr] \leq \mathcal{E}_\alpha(s)
...
$\mathbb{E}_\mathbb{P}[|\Theta|\gamma] = \mathbb{E}_\mathbb{Q}[|\Theta|] < \infty$, so
$\mathbb{E}_\mathbb{P}[\gamma\Theta\mid\mathcal{G}]$ is defined. Fix $G \in \mathcal{G}$. By the
defining property of conditional expectation under $\mathbb{P}$ and the definition of $\mathbb{Q}$,
\[
\int_G \mathbb{E}_\mathbb{P}[\gamma\Theta \mid \mathcal{G}]\,d\mathbb{P}
= \int_G \gamma\Theta\,d\mathbb{P} = \int_G \Theta \,d\mathbb{Q}
= \int_G \mathbb{E}_\mathbb{Q}[\Theta\mid\mathcal{G}]\,d\mathbb{Q}.
\]
On the other hand, writing $d\mathbb{Q} = \gamma\,d\mathbb{P}$ and using that
$\mathbb{E}_\mathbb{Q}[\Theta\mid\mathcal{G}]$ is $\mathcal{G}$-measurable, so that it may be taken
out of a conditional expectation given $\mathcal{G}$ (Proposition
\ref{def:conditional-expectation}, item 4),
\[
\int_G \mathbb{E}_\mathbb{Q}[\Theta\mid\mathcal{G}]\,d\mathbb{Q}
= \int_G \mathbb{E}_\mathbb{Q}[\Theta\mid\mathcal{G}]\,\gamma\,d\mathbb{P}
= \int_G \mathbb{E}_\mathbb{Q}[\Theta\mid\mathcal{G}]\;\mathbb{E}_\mathbb{P}[\gamma\mid\mathcal{G}]\,d\mathbb{P}.
\]
...
$\mathbb{E}_\mathbb{P}[\gamma\mid\mathcal{G}]$, which is strictly positive $\mathbb{Q}$-almost
...
$\mathcal{G} = \mathcal{F}_s$. Since $\mathcal{E}_\alpha$ is a $\mathbb{P}$-martingale,
$\mathbb{E}_\mathbb{P}[\mathcal{E}_\alpha(T)\mid\mathcal{F}_s] = \mathcal{E}_\alpha(s)$, so for
$s \leq t$
\begin{equation}
\mathbb{E}_\mathbb{Q}\bigl[X(t) \mid \mathcal{F}_s\bigr]
= \frac{\mathbb{E}_\mathbb{P}\bigl[X(t)\mathcal{E}_\alpha(T)\mid\mathcal{F}_s\bigr]}{\mathcal{E}_\alpha(s)} .
\end{equation}
The numerator simplifies by conditioning on $\mathcal{F}_t$ first and using that $X(t)$ is
$\mathcal{F}_t$-measurable:
\[
\mathbb{E}_\mathbb{P}\bigl[X(t)\mathcal{E}_\alpha(T)\mid\mathcal{F}_s\bigr]
= \mathbb{E}_\mathbb{P}\Bigl[X(t)\,\mathbb{E}_\mathbb{P}[\mathcal{E}_\alpha(T)\mid\mathcal{F}_t]\,\Big|\,\mathcal{F}_s\Bigr]
= \mathbb{E}_\mathbb{P}\bigl[X(t)\mathcal{E}_\alpha(t)\mid\mathcal{F}_s\bigr].
\]
Hence $\mathbb{E}_\mathbb{Q}[X(t)\mid\mathcal{F}_s] = X(s)$ if and only if
$\mathbb{E}_\mathbb{P}[X(t)\mathcal{E}_\alpha(t)\mid\mathcal{F}_s] = X(s)\mathcal{E}_\alpha(s)$,
...
Recall that, for a Markov process, the \textit{transition probability} $\mathbb P_{s,x}(t,A) = \mathbb P (X_t \in A \, | \, X_s = x)$, for $A$ measurable, $0\leq s\leq t \leq T$ and $x\in \mathbb{R}^n$. The Chapman-Kolmogorov equation states that 
$$
...
\begin{example}\label{example:bmisdiffusion}
    Take an $\mathbb{R}^n$-valued Brownian motion $W_t$. Note that $\lVert W_t\rVert^2$ is a sum of $n$ independent squares of normal variables, so that $\mathbb{E}\lVert W_\varepsilon\rVert^4 = n(n+2)\varepsilon^2$. Hence, for any $c>0$, Markov's inequality applied to the fourth power gives
    \[
    \mathbb{P}\bigl(\lVert W_{t+\varepsilon}-x\rVert\geq c \,\big|\, W_t=x\bigr)
    = \mathbb{P}\bigl(\lVert W_{t+\varepsilon}-W_t\rVert \geq c\bigr)
    \leq c^{-4}\,\mathbb{E}\lVert W_\varepsilon\rVert^4
...
The choice of space matters here: on the larger space $C_b[0,\infty)$ the same family fails to be
strongly continuous, since $\lVert T_tf - f\rVert_\infty \to 0$ is precisely uniform continuity of
$f$, which bounded continuous functions need not possess --- $f(x)=\sin(x^2)$ is the standard
witness. Restricting to $C_0$, or to the bounded uniformly continuous functions, repairs this.

Given a stationary diffusion $X_t$ driven by $\mu$ and $\Sigma$, one sets
$T_t(f)(x) = \int_{\mathbb{R}^n} f(y)\,\mathbb{P}_t(x,dy)$. Under mild regularity on the transition
...
contraction semigroup on $C_0(\mathbb{R}^n)$, the continuous functions vanishing at infinity, and its
...
Let $\mathbb{P}_{s,x}(t,\cdot)$ be the transition probabilities of a diffusion $X_t$ driven by $\mu$
