...
The asymmetry between the two halves is deliberate and is where the theorem earns its keep. In the converse direction one is \emph{not} allowed to assume that the limit $\charf$ is a characteristic function --- that is the conclusion, not a hypothesis. Continuity at the origin is what rules out mass escaping to infinity, and it is a genuine restriction on $\charf$; note that if one already knew $\charf$ to be a characteristic function the requirement would be vacuous, since characteristic functions are automatically uniformly continuous. It is precisely this form of the statement that makes the Central Limit Theorem below a two-line computation: one expands $\charf_{X_n}$, observes that the limit is $e^{-u^2/2}$, and only then concludes that the limit is Gaussian.
...
Let $W$ be a Brownian motion and write $\overline{W}_t := \max_{0\leq s\leq t} W_s$ for its
...
$\sqrt{t}$, on the same scale as the fluctuation itself. Second --- and this is the reason the
subsection is here --- \eqref{eq:jointmaxdensity} is a \emph{joint} law across the whole interval,
...
precisely because the payoff is convex, and the reason Proposition \ref{prop:condjensenl2}, item 8,
...
being a convex function of a martingale, by Proposition \ref{prop:condjensenl2}, item 8, together
