...
Chapter \ref{ch:foundations} built a machine and did not say much about what it is for. This
chapter gives the necessary financial background for using that machine. Everything below rests on the stochastic calculus developed there --- Itô's
rule, Girsanov's theorem and the martingale representation theorem are used constantly, and the
square-root process of Example \ref{example:cirwellposed} reappears as the variance of the Heston
model --- but the questions being asked are of a different kind. They are questions about
\emph{prices}, and the answers are constrained not by what is provable but by what the absence of
arbitrage permits.

The chapter is nonetheless mathematical throughout, and deliberately so. The claim we shall keep
returning to is that the financial content of this subject is not a layer of interpretation applied
on top of the probability: it \emph{is} the probability, read in a particular way. A change of
numeraire is a change of measure; a hedging strategy is the integrand supplied by a suitable representation
theorem; a risk-neutral density is a second derivative of observed option prices. 

\medskip

\section{Financial Derivatives}\label{sec:derivatives}

We first introduce the concept of a \textit{derivative} informally and give some examples of the main quoted contracts. The rigorous mathematical treatment of hedging and market completeness is addressed afterwards.
...
Let us fix a filtered probability space $\FPSpace$, where $\PP$ represents the physical (or real-world) probability measure and the filtration $\Filt$ models the flow of information over time up to a finite horizon $T$. Mathematically, a financial derivative with maturity $T$ is an $\mathcal{F}_T$-measurable random variable, denoted by $\Payoff_T$. A few contracts, such as perpetual American options, carry an infinite time horizon ($T=\infty$); these fall outside our finite-horizon framework and form no core part of the derivatives market, so we exclude them.

This measurability condition implies that the value of the derivative at maturity is fully determined by the information available at time $T$. The function determining this value is often referred to as the \textit{payoff function}, denoted by $\PayoffFn$, so that, for a claim written on the terminal spot, $\Payoff_T = \PayoffFn(\Spot_T)$. The key idea behind the subsequent theory is being able to assign \textit{fair prices} to those contingent claims, deduced from quoted (and liquid) market data.

The prices of such claims are not independent of one another. A small set of standardised, highly
liquid contracts carries enough information to determine the price of a great many other ones, and
...
for the fungible assets and electronic markets we shall be concerned with, the band is tight. Other underlyings (such as commodities) operate with larger spreads, so the models must be adapted to the context.
...
\begin{itemize}
    ...
    \end{equation}
        \item \textbf{American options:} as their European counterparts, except that the option may be \textit{exercised} at any time up to maturity $T$.
\end{itemize}

Derivatives enable \textbf{hedging}: they allow agents to transfer specific risks (e.g., volatility or tail risk) to those willing to bear them. In an idealised complete market, derivatives allow for the replication of any payoff profile. Some participants take a position for pure speculation, while others genuinely need to hedge, looking for a safer and more efficient way to transfer risk. Collectively, derivatives markets act as an information network: the price of a derivative encodes a \emph{risk-neutral} distribution for the underlying --- not the participants' forecast of what will happen, but that forecast reweighted by the price of bearing risk --- and Proposition \ref{prop:breedenlitzenberger} below makes that statement exact.
...
Let the market consist of $n+1$ assets denoted by the vector process $S = (S^0, S^1, \dots, S^n)$. We assume that each component $S^i$ is an \textbf{Itô process}. We distinguish the $0$-th asset, $S^0$, as the \textbf{numeraire}. While the most abstract fundamental theorems of asset pricing rely on the broader class of general semimartingales, Itô processes provide the precise mathematical framework required for the diffusion models prevalent in this dissertation.


\begin{definition}[Numeraire]
A numeraire is a  process $S^0$ that is strictly positive ($S^0_t > 0$ a.s. for all $t$) and non-dividend paying. It serves as the unit of account for the market.
\end{definition}
...
This is the point at which to record two objects that will be used constantly. \emph{Capitalising} one unit of currency forward from $t$ to $T$ at the short rate multiplies it
...
A \textbf{trading strategy} is a predictable process $\pi = (\pi^0, \pi^1, \dots, \pi^n)$, where $\pi^i_t$ represents the number of units of asset $i$ held at time $t$. The value of the portfolio at time $t$ is the scalar process:
\begin{equation}
...
A probability measure $\QQ$ is an \emph{equivalent local martingale measure} (ELMM) for the
numeraire $\Num$ if:
\begin{enumerate}
...
Assume the process $X$ is locally bounded. The market satisfies the NFLVR condition if and only if there exists at least one Equivalent Local Martingale Measure $\QQ$. \qedopentext
\end{theorem}
...
A market is \textbf{complete} if every claim $\Payoff_T \in L^\infty(\mathcal{F}_T)$ is \textit{attainable}. That is, for every bounded random variable $\Payoff_T$, there exists an admissible self-financing strategy $\pi$ and an initial capital $x \in \R$ such that:
\begin{equation}
...
This formulation reveals that ``risk-neutral probabilities'' are not absolute; they are tied to the choice of numeraire.
...
\begin{theorem}[Feynman--Kac Formula]\label{thm:feynmankac}
    ...
    \end{equation}
    with initial condition $X_t = x$, where $W$ is a standard $n$-dimensional $\QQ$-Brownian motion, and the diffusion matrix is given by $\Sigma(s, x) = \sigma(s,x)\sigma(s,x)\transp$. Let $\PayoffFn: \R^n \rightarrow \R$ be a continuous terminal payoff function --- continuity is forced by the regularity demanded of $u$ below --- and $\rf: [0,T] \times \R^n \rightarrow \R$ be a bounded continuous function representing the continuously compounded interest rate. Assume $u(t,x)$ is a sufficiently regular function (typically $u \in C^{1,2}([0,T) \times \R^n) \cap C([0,T] \times \R^n)$) that satisfies the following linear parabolic partial differential equation:
...
% emphatically not the physical $\PP$.

Two further remarks fix the place of the theorem. The first is that it subsumes material already
derived: the Kolmogorov backward equation \eqref{Kolmogorov_backward} of Chapter
\ref{ch:foundations} is exactly \eqref{FK_PDE} with $\rf \equiv 0$ and terminal condition
$\PayoffFn = \mathds{1}_{\{\,\cdot\, \le y\}}$, so the transition distribution function of a
diffusion is itself a Feynman--Kac representation --- one in which the ``payoff'' is the indicator of
ending below a level and the ``price'' is the probability of doing so. The second is what makes the
theorem a cornerstone of this chapter rather than a technical lemma. Read from
\eqref{FK_PDE} to \eqref{FK_Expectation} it turns a pricing equation into a discounted expectation
under a martingale measure; read in the opposite direction it turns the expectation
\eqref{eq:riskneutralpricing}, which is what the absence of arbitrage delivers, into a deterministic
PDE that the price must satisfy. 
...
$t$. Inside the model, $\sigma$ is the diffusion coefficient of the \emph{log}-price:
by Example \ref{example:blackscholessde}, $d\log \Spot_t = (\mu-\tfrac12\sigma^2)\,dt + \sigma\,d\Wt_t$,
so that $\Var[\log \Spot_t] = \sigma^2 t$. Within this subsection $\sigma$ is a \emph{parameter} of
the model and nothing else; the implied volatility of Definition \ref{def:impliedvol} is a separate
object, and Subsection \ref{subsec:impliedvol} keeps the two apart.
...
Consider a derivative whose value is a smooth function $V(t,\Spot)$ of time and the current spot, and
seek to \emph{replicate} it with $\Delta_t$ units of the underlying and $\beta_t$ units of the bank
account $\Num_t = e^{\rf t}$, held self-financingly in the sense of \eqref{eq:selffinancing}:
\[
\Pi_t = \Delta_t \Spot_t + \beta_t \Num_t,
\qquad
d\Pi_t = \Delta_t\,d\Spot_t + \beta_t\,d\Num_t = \Delta_t\,d\Spot_t + \rf\bigl(\Pi_t - \Delta_t\Spot_t\bigr)dt .
\]
Carrying the bank holding as the residual $\beta_t\Num_t = \Pi_t - \Delta_t\Spot_t$ is what keeps the
argument honest: the cost of rebalancing $\Delta$ is financed by $\beta$ rather than assumed away.
Requiring $\Pi_t = V(t,\Spot_t)$ at every $t$ then forces
\begin{equation}\label{eq:portfoliodynamics}
    dV_t = \Delta_t \, d\Spot_t + \rf\bigl(V - \Delta_t \Spot_t\bigr)\,dt .
\end{equation}
...
\begin{equation}\label{eq:portfoliogrouped}
    \underbrace{\left( \frac{\partial V}{\partial t}
    + \frac{1}{2}\sigma^2 \Spot_t^2 \frac{\partial^2 V}{\partial \Spot^2} - \rf V + \rf\Spot_t\Delta_t
    + \mu \Spot_t\Bigl[\frac{\partial V}{\partial \Spot} - \Delta_t\Bigr] \right)}_{\text{drift}} dt
    + \underbrace{\sigma \Spot_t\left( \frac{\partial V}{\partial \Spot} - \Delta_t \right)}_{\text{diffusion}} d\Wt_t \;=\; 0 .
\end{equation}
An Itô process that vanishes identically has both coefficients zero, a continuous local martingale of
finite variation being constant, so \eqref{eq:portfoliogrouped} splits into two equations. The
diffusion term forces
\begin{equation}\label{eq:delta}
    \Delta_t = \frac{\partial V}{\partial \Spot}(t,\Spot_t) ,
\end{equation}
which is the \emph{delta} of the derivative, and \eqref{eq:delta} is the hedge.
...
of the stock has cancelled}, so a position that is neutral to
the random part of $\Spot$ is automatically neutral to its drift as well. What remains of the drift is
...
\textbf{\textit{Proof:}} The hedging argument above. \qedfilledtext
...
\end{equation}
The passage from \eqref{eq:BS_SDE_P} to the dynamics in \eqref{eq:bsexpectation} is
Girsanov's theorem with $-\alpha = (\mu-\rf)/\sigma$ the
\emph{market price of risk}: the measure
change subtracts a drift and leaves $\sigma$ untouched. Its hypothesis is met here without effort: $\alpha$ is a constant, so
$\E\bigl[\exp\bigl(\tfrac12\int_0^T\alpha^2\,ds\bigr)\bigr] = e^{\alpha^2T/2} < \infty$ and
Novikov's condition (Theorem \ref{thm:novikov}) holds, whence $\mathcal{E}_\alpha$ is a genuine
martingale and $\QQ$ is a probability measure equivalent to $\PP$. That the hedging argument and the martingale argument give the same answer is 
not a coincidence but the content of Feynman--Kac.
...
integrand $\pi$ whose abstract existence the representation theorem guarantees; here we have it
explicitly.
...
economically meaningful. We do not include the derivation here, but the Bachelier formula for a call is
\begin{equation}\label{eq:bacheliercall}
    C_{\mathrm{Bach}}(t,\Spot) = (\Spot-\Strike) N(d) + \sigma\sqrt{\tau} N'(d), \qquad d = \frac{\Spot-\Strike}{\sigma\sqrt{\tau}},
\end{equation}
with $N'$ the standard normal density. The Bachelier formula is a special case of the general Feynman--Kac representation \eqref{FK_Expectation} with $\rf \equiv 0$ and $\PayoffFn(\Spot) = (\Spot-\Strike)^+$.

\subsubsection*{The Greeks}

The delta \eqref{eq:delta} was singled out above as the hedge, and identified in Remark
\ref{rem:mrtpricing} with the integrand $\pi$ whose existence the Martingale Representation
...
$\Gamma$ are in exact opposition. This is the whole economics of running an option book:
...
Vega is strictly positive, so a call is worth more when volatility is
...
constant is a symptom that the model is being used outside its own assumptions. 


\subsection{The Forward and Put--Call parity}\label{subsec:forward}
...
by $\rf - \divy$, and $\rf$ by $\rf-\divy$ in $d_\pm$. Nothing structural changes and the discounted-spot term acquires its 
own factor:
...
Actual equity dividends are \emph{discrete cash and proportional amounts}, announced in advance and paid on known 
dates. If a cash amount $D_i$ is paid at time $t_i$ together with a proportional dividend $d_i$, then no-arbitrage forces the spot to drop by that 
amount across the payment:
\begin{equation}\label{eq:dividendjump}
\Spot_{t_i} = (1-d_i) \Spot_{t_i^-} - D_i .
\end{equation}
This is genuinely destructive of the framework above.
Equation \eqref{eq:dividendjump} says $\Spot$ has a \emph{predictable jump}: the process is not 
continuous, so Itô's rule as stated does not apply across $t_i$. Worse, one 
cannot write $\Spot_t = \Spot_0 \mathcal{E}(\cdot)$ for a positive $\mathcal{E}$ and reproduce 
\eqref{eq:dividendjump}. Also, a fixed cash dividend can in principle exceed the spot price, 
...
price at $t$ of a default-free zero-coupon bond maturing at $T$,
...
Identity \eqref{eq:forwarddef} says that the forward price is a $\QT$-martingale. It holds by construction, with no assumption 
...
announced dividends. Their precise construction is a market-data question, not a modelling
...
For a European claim at a \emph{single} maturity the change of variable costs nothing at all: since
$\Fwd(T,T)=\Spot_T$, the payoff $(\Spot_T-\Strike)^+$ \emph{is} $(\Fwd_T-\Strike)^+$, so one may price
the call on the forward --- which is continuous and log-normal --- rather than on the spot, which is
neither. The calibration is therefore much simpler, since we only need to work with the forward
implied volatility. The result is the celebrated Black-76 formula \cite{black1976}.

Across a whole \emph{surface} the relation \eqref{eq:spotforwardfactor} does substantive work, and it
is worth being explicit that its coefficients carry the maturity as well as the observation time:
each quoted maturity has its own forward, $\Fwd(t,T)=a(t,T)\,\Spot_t+b(t,T)$, and at $t=0$ not one of
them is the spot. Two consequences follow. The log-moneyness
$\lmon=\log(\Strike/\Fwd(t,T))$ of \eqref{eq:totalvariance} centres every smile on its
own forward, so the forward curve is what aligns the surface in the strike direction: an error in it
shears the smiles against one another by an amount that grows with $T$, and Subsection
\ref{subsec:rhresults} uses exactly that signature to rule out a spot-moneyness convention in the
data. And it is what leaves the local volatility clean, since the forward dynamics
\eqref{eq:localvolforward} carry no drift, and with it none of the rate and dividend terms that
\eqref{eq:dupire} does. Finally, for a contract looking at the spot \emph{strictly before} $T$ the
relation is needed inverted, $(\Spot_t-\Strike)^+=\bigl(\Fwd_t-(a_t\Strike+b_t)\bigr)^+/a_t$: a call
on the forward struck at $a_t\Strike+b_t$, which is the American obstacle
\eqref{eq:americanobstacle} of Subsection \ref{subsec:american}.
...
$\Bond{t}{T}(\Fwd(t,T)-\Strike)$ by \eqref{eq:forwarddef}. \qedfilledtext

The proof used no dynamics, only 
...
sanity check on any implementation: a pricer that violates put--call parity is wrong.

\subsection{Implied Volatility and the Inversion Problem}\label{subsec:impliedvol}
In professional markets, participants do not ``price'' options using Black--Scholes to find the dollar value. The market price $C^{\mathrm{mkt}}$ is determined by supply and demand. Instead, Black--Scholes is used as a translation mechanism. Since vega is strictly positive (Proposition \ref{prop:greeks}), the pricing function $\sigma \mapsto C_{\mathrm{BS}}(\sigma)$ is strictly \emph{increasing}, and it sweeps out the whole no-arbitrage interval as $\sigma$ runs over $(0,\infty)$; any quote inside that interval therefore has exactly one preimage.

\begin{definition}[Implied volatility]\label{def:impliedvol}
Let $C^{\mathrm{mkt}}$ be an observed European call price with strike $\Strike$ and maturity $T$,
lying strictly inside the no-arbitrage interval. Its \textbf{implied volatility}
$\ivol(\Strike,T)$ is the unique $\sigma > 0$ solving
\begin{equation}\label{eq:impliedvoldef}
    C_{\mathrm{BS}}(\Spot, \Strike, T, \rf, \divy, \sigma) = C^{\mathrm{mkt}},
\end{equation}
the dividend yield $\divy$ entering through \eqref{eq:bscalldiv}. The map
$(\Strike,T)\mapsto\ivol(\Strike,T)$ is the \textbf{implied volatility surface}, and it is the object
Chapter \ref{ch:applications} calibrates against.
\end{definition}

Implied volatility, so defined, is the true ``price'' quoted in the market: a trader who says an
option ``trades at fifteen'' is quoting $\ivol$, and the dollar amount is recovered from
\eqref{eq:impliedvoldef}. Note what the definition does \emph{not} claim. It does not assert that the
underlying follows \eqref{eq:BS_SDE_P}; it uses $C_{\mathrm{BS}}$ only as a strictly monotone change
of units, and it would remain well posed even if every modelling assumption of Subsection
\ref{subsec:bs_model} were false --- as, by the next subsection, they are.

\subsubsection*{Implied against realised volatility}

The word ``volatility'' is by now doing three quite different jobs, and since the remainder of the
dissertation turns on telling them apart, we separate them here once and for all. There is $\sigma$,
a \emph{parameter} of the model \eqref{eq:BS_SDE_P}, constant by assumption and not observable.
There is $\ivol$, a \emph{quote}: read off an observed price through Definition
\ref{def:impliedvol}, and varying with strike and maturity precisely because the model that defines
it is false. And there is a \emph{statistic} of the trajectory that actually occurred, which is the
object we now make precise.

\begin{definition}[Realised volatility]\label{def:realisedvol}
Suppose the log-price has instantaneous variance $\sigma_t^2$, so that
$d\log\Spot_t = \mu_t\,dt + \sigma_t\,d\Wt_t$ for some adapted drift $\mu_t$, which plays no part in
what follows. The \textbf{realised variance} over $[0,T]$ is the
time average
\begin{equation}\label{eq:realisedvariance}
\rvol^2[0,T] := \frac{1}{T}\int_0^T \sigma_t^2\,dt = \frac{1}{T}\,\bigl\langle \log \Spot \bigr\rangle_T ,
\end{equation}
and $\rvol$ is the \textbf{realised volatility}. Given prices $\Spot_0,\dots,\Spot_n$ observed at
spacing $\Delta = T/n$, it is estimated by
\begin{equation}\label{eq:realisedestimator}
\widehat{\sigma}^{\,2} = \frac{1}{T}\sum_{i=1}^{n}\Bigl(\log\frac{\Spot_i}{\Spot_{i-1}}\Bigr)^{\!2} .
\end{equation}
Two conventions are worth making explicit. No mean is subtracted, the drift contributing only
$O(\Delta)$ to each term; and the normalisation is by $T$ rather than by the number of observations,
so that $\widehat{\sigma}^{\,2}$ estimates a variance \emph{per unit time} and is therefore on the
same footing as $\ivol^{2}$.
\end{definition}

The second equality in \eqref{eq:realisedvariance} is not a definition but a theorem of Chapter
\ref{ch:foundations}: by Itô's rule the quadratic variation of $\log\Spot$ is
$\int_0^T\sigma_t^2\,dt$, and by Definition \ref{def:quadratic-variation} and Proposition
\ref{prop:qvcoincide} that quantity is the limit of the sums of squared increments as the mesh
refines. The estimator \eqref{eq:realisedestimator} is therefore not a heuristic: it is the pathwise
object of Subsection \ref{subsec:martingales} evaluated on data, and its convergence as
$\Delta \to 0$ is Proposition \ref{prop:qvcoincide} applied to $\log\Spot$.


Realised volatility is a functional of the trajectory that actually
    occurred: a statistic under $\PP$. Implied volatility is extracted from a \emph{price}, and
    prices are expectations under $\QQ$ by \eqref{eq:riskneutralpricing}. By Girsanov's theorem
    (Theorem \ref{girsanovtheorem}) the two measures agree on the diffusion coefficient of any
    \emph{given} model
    but they do not agree on the price of \emph{bearing} volatility, and that price is inside
    $\ivol$.
...
If the assumptions of Black--Scholes were satisfied physically (i.e., if returns were truly log-normal with constant variance), the implied volatility $\ivol$ would be constant across all strike prices $K$ and maturities $T$. Empirical data reveals this is not the case. When we plot $\ivol(K)$ against $K$, we observe distinct shapes:
\begin{itemize}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{Images/smile-skew.pdf}
\caption[The skew and the smile]{The two shapes described above, each drawn against the flat line
that Black--Scholes predicts. Left: the typical equity skew, falling monotonically in the strike, so
that downside protection carries the highest implied volatility. Right: the usual foreign exchange
smile, near-symmetric about the money and rising into both wings. The curves are illustrative shapes
and not drawn from market quotes.}
\label{fig:smileskew}
\end{figure}

The two shapes, drawn in Figure \ref{fig:smileskew}, are not merely descriptive. Both say that the
market's risk-neutral distribution is not log-normal, and the following classical result makes the
connection exact.
...
suppose the law of $\Spot_T$ under the $T$-forward measure $\QT$ admits a density $p_T$ --- the
risk-neutral density itself whenever rates are deterministic, since $\QT = \QQ$ in that case. Then
\begin{equation}\label{eq:breedenlitzenberger}
    p_T(\Strike) = \frac{1}{\Bond{0}{T}}\,\frac{\partial^2 C}{\partial \Strike^2}(\Strike,T).
\end{equation}
\end{proposition}

\textbf{\textit{Proof:}} Write $C(\Strike,T) = \Bond{0}{T}\int_{\Strike}^{\infty}(x-\Strike)p_T(x)\,dx$. 
Differentiating once under the integral sign, the boundary term vanishes because the integrand is 
zero at $x=\Strike$, thus obtaining 
$\partial_\Strike C = -\Bond{0}{T}\int_\Strike^\infty p_T(x)\,dx$. Differentiating a second time 
gives \eqref{eq:breedenlitzenberger}. \qedfilledtext
...
$p_T \geq 0$, arbitrage-free call prices must be \emph{convex} in strike; a violation is a 
butterfly arbitrage, executable by buying two wing options and selling two at the middle strike. 
Second, the formula involves a \emph{second} derivative of market data, which is a numerically 
delicate problem.
...
\section{Other Relevant Models}\label{sec:othermodels}
...
Therefore, there is 
exactly one local volatility diffusion consistent with a given arbitrage-free implied volatility surface. The 
...
time  and two in strike of raw market data, and market data are quoted on a sparse grid, carry 
...
\begin{itemize}
    ...
    $|\lmon|\to\infty$; Lee's moment formula \cite{lee2004} bounds the asymptotic slopes by $2$, this
    being a \emph{necessary} condition on an arbitrage-free surface rather than a sufficient one. What
    is required in addition, in the Gatheral--Jacquier formulation, is the tail condition
    $d_{+}(\lmon)\to-\infty$ as $\lmon\to+\infty$, which is what forces the call price to vanish in the
    far wing.
\end{itemize}
\qedopentext
\end{proposition}
...
constrains the extrapolated wings: by Lee's formula an asymptotic slope of $2$ is the borderline at
which moments of $\Spot_T$ cease to exist.

In practice one first fits a parametric form designed to respect i)--iii) --- the SVI
parameterisation \cite{gatheral2014} is the standard choice, with SABR common in rates
markets --- and only then differentiates. Neither guarantees ii) of itself: raw SVI can admit
butterfly arbitrage, and it is the constrained variants that exclude it. Since the fitted surface is smooth and arbitrage-free by construction, the 
differentiation is stable and the local variance is automatically non-negative.
...
$(\kappa,\theta,\xi,\rho,v_0)$ directly to liquid quotes. The incompleteness is not irreparable:
admitting a single traded option as a second instrument completes the market again, and the same
argument yields the representation of a European price as a mixture of Black--Scholes values
\eqref{eq:bscall} taken over the realised variance, obtained by conditioning on the variance path.
Both are due to Romano and Touzi \cite{romanotouzi1997}, and the second is the standard route to
prices in a stochastic volatility model when no characteristic function is available.
...
elementary functions. \qedopentext
\end{theorem}
...
% variable in Subsection \ref{subsec:forward}.
...
recovered from the characteristic function through an integral of Lewis--Carr--Madan type \cite{lewis2001,carrmadan1999},
...
against prices: both errors are otherwise silent. In that limit the variance is deterministic,
$v_t = \theta + (v_0-\theta)e^{-\kappa t}$, so the comparison must be made at the implied volatility
$\sigma^2 = \bigl[\theta\tau + (v_0-\theta)(1-e^{-\kappa\tau})/\kappa\bigr]/\tau$ and not at an
arbitrarily chosen constant.

One implementation caveat deserves a line. The 
...
corrupting prices at long maturities while leaving short ones correct. We shall write the solution in the form whose principal branch is 
...
We conclude this chapter by  discussing the challenges posed by path-dependent derivatives, focusing on the most liquid and prominent example: American options. 

Because an American option may be exercised at any time up to maturity, its value depends on the law of the underlying at \emph{every} intermediate date, not merely at $T$. A single implied volatility, which by Definition \ref{def:impliedvol} is one number read off one European quote, therefore does not determine it: infinitely many dynamics reproduce that quote while disagreeing about the intermediate marginals, and hence about the exercise boundary. The point matters in practice because options on \textit{single names} (i.e., individual corporate stocks) are almost exclusively American-style. Investors in single equities are reluctant to relinquish the flexibility of early exercise, particularly to capture discrete dividends or navigate corporate actions. In contrast, broad market indices are generally considered more stable and are traded via extremely liquid European options.
...
The result below is due to Merton \cite{merton1973}, and it explains why dividends have kept
reappearing in the discussion above. Its proof refers forward to the optimal stopping problem
\eqref{eq:optimalstopping}, written out two subsections below; the reader willing to grant that an
American option is worth the supremum, over exercise times, of the expected discounted payoff will
lose nothing by reading it here.
...
(Proposition \ref{prop:condexpproperties}, item 5) applied to the convex function $x \mapsto (x-\Strike)^+$,
\[
...
being a convex function of a martingale, by Proposition \ref{prop:condexpproperties}, item 5, together
...
\]
Since $\tau \geq t$ we have $\Ft \subseteq \mathcal{F}_\tau$, so conditioning both sides on $\Ft$ and applying the tower property (Proposition \ref{prop:towerl2}, item i)) gives
\[
\Ep{\QQ}\bigl[e^{-\rf\tau}(\Spot_\tau-\Strike)^+ \,\big|\, \Ft\bigr]
\;\leq\; \Ep{\QQ}\bigl[e^{-\rf T}(\Spot_T-\Strike)^+ \,\big|\, \Ft\bigr] ,
\]
so the supremum in \eqref{eq:optimalstopping} is attained at $\tau = T$. \qedfilledtext
...
exercise early: one would be surrendering the interest earned on the strike.  For the American \emph{put} the argument fails at the first step regardless of
dividends: the same Jensen bound yields only
$P(t) \geq \bigl(\Strike e^{-\rf(T-t)}-\Spot_t\bigr)^{+}$, which lies \emph{below} the
immediate-exercise payoff $(\Strike-\Spot_t)^{+}$, and early exercise is indeed optimal for low
enough spot. Note also that put--call parity, Proposition \ref{prop:putcallparity}, has no exact
American analogue; only the bounds
$\Spot_t - \Strike \leq C_A - P_A \leq \Spot_t - \Strike e^{-\rf(T-t)}$ survive.

We aim to show how a local volatility model can be adapted to price American options. However, we must first note a fundamental limitation: Local Volatility models do not perform perfectly for all types of path-dependent derivatives. In particular, because local volatility models make the volatility a deterministic function of the spot, hence perfectly correlated with it, the future smile is determined by where the spot goes, and the model misprices derivatives whose strikes are set at a future date, such as forward-starting options and cliquets. 
...
measure of Subsection \ref{subsec:forward}, is
\begin{equation}\label{eq:localvolforward}
    d\Fwd_t = \lvol(t,\Fwd_t)\,\Fwd_t \, d\Wt^{T}_t .
\end{equation}
% Working in the forward has the same three advantages set out in Subsection \ref{subsec:forward}: 
...
problem, and its solution is classical. Discounting has to be brought inside before the classical statement applies, since it is the
...
Let $Y$ be a right-continuous adapted process of class D (see Definition \ref{def:classD}). Then
...
\end{itemize}
and, \emph{if $Y$ is in addition continuous}, the stopping time
$\tau^{*} = \inf\{ s \geq t : \Snell_s = Y_s \}$ is optimal in \eqref{eq:optimalstopping}. A full
treatment is given in \cite{peskirshiryaev2006}. \qedopentext
\end{theorem}

Continuity is what rules out the reward falling away at the instant it is reached; without it the
first contact need not be optimal, and only the envelope itself survives. It holds for the obstacle
\eqref{eq:americanobstacle} whenever the coefficients $a_t$ and $b_t$ of
\eqref{eq:spotforwardfactor} are continuous.
...
says that this cost is zero exactly where continuation is optimal. The optimal rule is then transparent: continue while 
...
$\mathfrak{L}$ for the pricing operator associated with \eqref{eq:localvolforward},
\[
...
\eqref{eq:americanobstacle}. \qedopentext
\end{theorem}
