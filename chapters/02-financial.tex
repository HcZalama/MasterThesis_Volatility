...
theorem; a risk-neutral density is a second derivative of observed option prices. Where that
correspondence is exact we say so, and where the modelling outruns what has been proved --- as it
does, in a precise and instructive way, at the end of Section \ref{sec:othermodels} --- we say that
too, because it is where Part \ref{part:part2} begins.
...
Assume that the set of equivalent local martingale measures $\mathcal{M}^e$ is non-empty (i.e., the market is arbitrage-free). The market is complete if and only if $\mathcal{M}^e$ is a singleton set (i.e., $\QQ$ is unique). \qedopentext
\end{theorem}
...
PDE that the price must satisfy. The two descriptions of a derivative's value --- the probabilistic
one and the analytic one --- are therefore the same object seen from two sides, and both directions
will be used below.
...
\ref{girsanovtheorem}. Its hypothesis is met here without effort, and it is worth checking once
rather than assuming it: $\alpha$ is a constant, so
...
In professional markets, participants do not ``price'' options using Black--Scholes to find the dollar value. The market price $C^{\mathrm{mkt}}$ is determined by supply and demand. Instead, Black--Scholes is used as a translation mechanism. Since vega is strictly positive (Proposition \ref{prop:greeks}), the pricing function $\sigma \mapsto C_{\mathrm{BS}}(\sigma)$ is strictly \emph{increasing}, and it sweeps out the whole no-arbitrage interval as $\sigma$ runs over $(0,\infty)$; any quote inside that interval therefore has one and only one preimage.
...
$\Delta \to 0$ is exactly \eqref{eq:bmqv}. That the non-vanishing quadratic variation which forced
It\^o's construction is also what makes volatility \emph{measurable} from a price series is not a
coincidence; it is the same fact seen twice.

The two quantities differ in three respects, and conflating them is the commonest error in this
subject.

\begin{itemize}
    \item[i)] \textbf{Measure.} Realised volatility is a functional of the trajectory that actually
    ...
    \emph{given} model --- which is why volatility, unlike expected return, is estimable at all ---
    but they do not agree on the price of \emph{bearing} volatility, and that price is inside
    $\ivol$.
    \item[ii)] \textbf{Direction in time.} Realised volatility looks backwards over a window that has
    already elapsed. Implied volatility looks forward over $[t,T]$, and summarises the market's view
    of the root-mean-square volatility over the life of the contract.
    \item[iii)] \textbf{What they are attached to.} Realised volatility is a single number attached
    to a path and a window. Implied volatility is attached to a \emph{contract}: it depends on
    $\Strike$ as well as on $T$, and is therefore a surface rather than a number.
\end{itemize}

Item iii) is the decisive one. Were Black--Scholes true, the surface would be flat and all three
notions --- the parameter $\sigma$, the realised volatility $\rvol$, and the implied volatility
$\ivol(\Strike,T)$ --- would be the same number. Empirically the surface is not flat, and the gap
between the second and the third is a systematic one: implied volatility exceeds subsequently
realised volatility on average, the difference being the \emph{variance risk premium} demanded by
whoever sells the option and carries the risk. Implied volatility is consequently a price and not a
forecast, and a model calibrated to option quotes is calibrated under $\QQ$. Everything from the
next subsection onwards is an attempt to build dynamics under which the surface
$\ivol(\Strike,T)$ can be non-flat and still arbitrage-free.
...
is not, and early exercise is optimal for low enough spot. Note also that put--call parity, Proposition \ref{prop:putcallparity}, has no American analogue: its proof rests on an identity between payoffs at a \emph{single} date, which early exercise destroys.
