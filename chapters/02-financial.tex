...
Chapter \ref{ch:foundations} built a machine and said almost nothing about what it is for. This
chapter is the answer. Everything below rests on the stochastic calculus developed there --- Itô's
...
A trading strategy is a predictable process $\pi = (\pi^0, \pi^1, \dots, \pi^n)$, where $\pi^i_t$ represents the number of units of asset $i$ held at time $t$. The value of the portfolio at time $t$ is the scalar process:
\begin{equation}
...
First, let's avoid confusing the several roles of volatility. If we take market prices $\Spot_0,\Spot_1,\dots,\Spot_n$ are observed at
spacing $\delta t$, the log-returns given by $R_i := \log(\Spot_i/\Spot_{i-1})$ are \emph{not} distributed as in
\eqref{eq:BS_SDE_P}, independent and $\Normal\bigl((\mu-\tfrac12\sigma^2)\delta t,\ \sigma^2\delta t\bigr)$. We call the volatility estimated from these log-returns the \emph{historical} or \emph{realised} volatility:

\begin{equation}
\sigma_{hist}^2 := \frac{1}{n-1}\sum_{i=1}^n (R_i - \bar{R})^2, \qquad \bar{R} := \frac{1}{n}\sum_{i=1}^n R_i.
\end{equation}

The following model is a stylised representation of the underlying's dynamics, and the volatility $\sigma$ appearing in it is not directly observable. Instead, it is inferred from market prices of derivatives, particularly options, and is known as the \emph{implied} volatility. The implied volatility is the value of $\sigma$ that, when input into the Black--Scholes formula, yields a theoretical option price equal to the observed market price.
...
so that $\Var[\log \Spot_t] = \sigma^2 t$. We call $\sigma$ the \emph{implied} volatility.
...
\subsection{The Forward and Put--Cal parity}\label{subsec:dividends}
...
price at $t$ of a (deterministic) zero-coupon bond maturing at $T$,
...
The word ``volatility'' is now doing two quite different jobs, and since the remainder of the
dissertation turns on the difference, we separate them explicitly. Subsection \ref{subsec:bs_model}
made the first distinction, between $\sigma$ as a \emph{parameter} of the model and $\sigma$ as a
\emph{statistic} of observed returns. The second object is the statistic, made precise.
...
$\Delta \to 0$ is exactly \eqref{eq:bmqv}.
...
In practice one first fits a parametric form guaranteeing i)--iii) --- SVI
parameterisation \cite{gatheral2014} is the standard choice, with SABR common in rates
markets --- and only then differentiates. Since the fitted surface is smooth and arbitrage-free by construction, the 
...
We conclude this chapter by briefly discussing the challenges posed by path-dependent derivatives, focusing on the most liquid and prominent example: American options. 
...
is not, and early exercise is optimal for low enough spot. Note also that put--call parity, Proposition \ref{prop:putcallparity}, has no American analogue.
...
\eqref{eq:optimalstopping}. A full treatment is given in \cite{peskirshiryaev2006}. \qedopentext
\end{theorem}
