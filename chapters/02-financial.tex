...
numeraire is a change of measure; a hedging strategy is the integrand supplied by a representation
...
\subsection{The Forward and Put--Call parity}\label{subsec:dividends}
...
Thus, we can transform the formula $(S_t-K)^+ = ((F_t-b_t)/a_t-K)^+ = (F_t-(K a_t+b_t))^+/a_t$ and price the call on the forward, which is continuous and log-normal, rather than on the spot, which is not. The calibration is therefore much simpler, since we only need to work with the forward implied volatility. The result is the celebrated Black-76 formula \cite{black1976}. 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{Images/smile_skew.png}
\caption[The skew and the smile]{The two shapes described above. Left: the usual foreign exchange smile,
symmetric about the money and rising into both wings. Right: the typical equity skew, falling monotonically in the strike, so that
downside protection carries the highest implied volatility.  The curves are illustrative shapes and not drawn from market quotes.}
\label{fig:smileskew}
\end{figure}
