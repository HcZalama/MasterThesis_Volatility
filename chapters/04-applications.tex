...
\begin{itemize}
  ...
  \item[ii)] positive curvature \emph{raises} $g$, so curvature is a cost only where it is
    ...
    finite asymptote must approach it concavely. At the join this is $\ivd{kk}=-\nskew/\ell<0$,
...
The proof is given in Appendix \ref{app:proof:master}.  Under \eqref{eq:ellcap} the lift obeys $b=\nskew_{0}a\ell/|\lmon^{*}|\le
...
$\upsilon_{2}<0$ and a maximum iff $\upsilon_{2}>0$; in the latter case, when $\upsilon_{2}=0$ and
$\hat{x}^{*}$ is undefined, or when $\hat{x}^{*}\notin[0,\infty)$, the minimum is attained at an
endpoint. Hence
...
the learning-rate schedule. Finally, a \emph{ratchet} snapshots the parameters $\Theta$ at the end of each rotary period and keeps the best state seen. 
...
condition is measured cell by cell, and the weight carried by that condition on that cell is raised
in proportion to the measured violation. The weights are therefore a function of position on the
...
$c_{\mathrm{soft}}$ & $60$ & Softmin sharpness in Remark \ref{rem:softmin}; error
  $\le\log2/60\approx0.0116$, an absolute error rather than a relative one, absorbed by
  $c_{\dagger}$; the soft minimum lies \emph{below} the exact one, so it cannot breach a ceiling but
  can breach a floor.\\
...
The tests performed, mainly for arbitrage purposes, have been passed. In terms of market fit, the main limitation has been noisy quotes as input data. Figure \ref{fig:pinnsurface} above shows the fitted surface against the quotes. It belongs to the SPX index, which is in a sense one of the most complex underlyings to calibrate due to the abundance of market data, tight bid/ask spreads and the presence of a strong skew. The errors measured to the mid are typically below five basis points, and $550$ of the $576$ quotes fall inside their own bid/ask, which is a good outcome given the complexity of the surface and the number of quotes fitted. The surface is smooth, and the implied volatilities are consistent with market observations.
...
The book is the one described in \S\ref{sec:pinn}, but the sample and the objective are not those
of \S\ref{subsec:objective}, and the two studies are therefore not a controlled comparison. Two
...
the bound on the polish is not a detail.
...
$\xi$ & $0.8968$ & \\
$\rho$ & $-0.7346$ & \\
$\Hurst$ & $0.4900$ & \emph{at the smooth end} of the box $[0.02,\,0.50]$; $\alpha_{\Hurst}=0.99$\\
\hline
...
\hline
\end{tabular}
\caption{Rough Heston calibrated to $2415$ SPX quotes, all six
...
$\Hurst$ at all}. What the monotone profile reflects is the short-dated at-the-money term structure,
which a one-factor rough Heston with a flat $v_{0}$ and $\theta$ cannot carry; $\Hurst$ is left to
absorb that misspecification.
...
cannot tell them apart. The legend of Figure \ref{fig:rhidentification} reports $-0.506$ for the
last of these, the same fit recomputed on the plotted maturity grid; the two agree to the precision
...
The model with $\Hurst=0.49$ --- numerically close to the classical endpoint --- reproduces the
...
off the rough model is $-0.007$, outside the admissible range $(0,\tfrac12)$ altogether. That is not
...
coupling enters through a coefficient $\theta$ rather than through a time shift; the two are not the
...
The scan behaves as expected. Every non-zero lag is detected at
...
hours apart, which is manifestly a lead--lag --- is \emph{not} disposed of, and the limitation
should be stated plainly. A displacement shorter than the sampling interval does not leave the
expected area unchanged. For two-sided Brownian drivers observed at unit spacing with an offset
$\delta\in(0,1)$, the linearly interpolated pair has $\E[A]=\delta/2$ rather than $0$, because the
overlap of the two increments contributes $\delta$ to one cross-covariance and nothing to the other.
The closing offset is therefore a genuine confound of the daily measurement rather than something
the statistic may ignore, and resolving it requires intraday sampling, at which point the offset
moves inside the observable range. 
...
Fourier route from a characteristic function to a price is
